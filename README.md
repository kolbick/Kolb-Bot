# Kolb-Bot Fork Build Specification

This README is the authoritative implementation brief for the Kolb-Bot repository.

## Product definition

Build **Kolb-Bot**, a completely branded, private, self-hosted AI workspace derived from Open WebUI.
...
The Compose project must use unique cookie names, session secrets, encryption keys, cache prefixes, browser-storage keys, database paths, service names, network names, and volumes beginning with `kolb-bot` or another clearly Kolb-specific namespace.
...
The Kolb-Bot build is complete only when all of the following are true:

- A production build runs from this repository in its own Docker Compose stack.
- The product is reachable through a reverse proxy at `kolb-bot.com`.
- No unintended Open WebUI name, logo, promotional link, or visual identity appears anywhere in the user-facing application.
- No Tide-Bot name, domain, logo, copy, storage key, or container resource appears.
- All required upstream licenses and notices remain intact.
- Kolb Terminal is built from source, branded, isolated, authenticated, healthy, and integrated.
- Kolb Computer is configurable through the CPTR gateway without altering CPTR's own required attribution.
