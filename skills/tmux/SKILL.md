...
```bash
SOCKET_DIR="${KOLB_BOT_TMUX_SOCKET_DIR:-${CLAWDBOT_TMUX_SOCKET_DIR:-${TMPDIR:-/tmp}/kolb-bot-tmux-sockets}}"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/kolb-bot.sock"
SESSION=kolb-bot-python
...
- Use `KOLB_BOT_TMUX_SOCKET_DIR` (legacy `CLAWDBOT_TMUX_SOCKET_DIR` also supported).
- Default socket path: `"$KOLB_BOT_TMUX_SOCKET_DIR/kolb-bot.sock"`.
...
- Scan all sockets: `{baseDir}/scripts/find-sessions.sh --all` (uses `KOLB_BOT_TMUX_SOCKET_DIR`).
