---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for KolbBot deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, KolbBot cron scheduling for periodic checks, or version status checks on a machine running KolbBot (laptop, workstation, Pi, VPS).
---

# KolbBot Host Hardening

## Overview

Assess and harden the host running KolbBot, then align it to a user-defined risk tolerance without breaking access. Use KolbBot security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim KolbBot changes the host firewall, SSH, or OS updates; it does not.
...
5. KolbBot gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if KolbBot can access sensitive data.
...
### 2) Run KolbBot security audits (read-only)
...
Offer to apply KolbBot safe defaults (numbered):

1. `kolb-bot security audit --fix`

Be explicit that `--fix` only tightens KolbBot defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check KolbBot version/update status (read-only)
...
- Credential hygiene notes (location of KolbBot creds, prefer disk encryption)
...
After KolbBot install or first hardening pass, run at least one baseline audit and version check:

- `kolb-bot security audit`
- `kolb-bot security audit --deep`
- `kolb-bot update status`

Ongoing monitoring is recommended. Use the KolbBot cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
Do not invent CLI flags or imply KolbBot enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by KolbBot compaction:
