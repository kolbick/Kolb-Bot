...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        KOLB_BOT_GATEWAY_TOKEN: inst.gatewayToken,
        KOLB_BOT_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          KOLB_BOT_GATEWAY_PORT: String(gwA.port),
          KOLB_BOT_GATEWAY_TOKEN: gwA.gatewayToken,
          KOLB_BOT_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          KOLB_BOT_GATEWAY_PORT: String(gwB.port),
          KOLB_BOT_GATEWAY_TOKEN: gwB.gatewayToken,
          KOLB_BOT_GATEWAY_PASSWORD: "",
        }),
