...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("kolb-bot-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("kolb-bot-bin-sherpa-");
      const modelDir = await makeTempDir("kolb-bot-sherpa-model-");
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("kolb-bot-bin-whispercpp-");
      const modelDir = await makeTempDir("kolb-bot-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("kolb-bot-bin-gemini-");
      ...
      const cfg: KolbBotConfig = { tools: { media: { image: {} } } };
