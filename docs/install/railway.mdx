...
Deploy KolbBot on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/kolb-bot-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/kolb-bot` — Control UI

## What you get

- Hosted KolbBot Gateway + Control UI
...
- `KOLB_BOT_STATE_DIR=/data/.kolb-bot` (recommended)
- `KOLB_BOT_WORKSPACE_DIR=/data/workspace` (recommended)
- `KOLB_BOT_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your KolbBot state + workspace so you can migrate to another host without losing config or memory.
