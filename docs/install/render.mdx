...
Deploy KolbBot on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/kolb-bot/kolb-bot)
...
repository configures everything needed to run KolbBot:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: KOLB_BOT_STATE_DIR
        value: /data/.kolb-bot
      - key: KOLB_BOT_WORKSPACE_DIR
        value: /data/workspace
      - key: KOLB_BOT_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/kolb-bot`.
...
If you use the original KolbBot repository, Render will not auto-deploy your KolbBot. To update it, run a manual Blueprint sync from the dashboard.
...
For KolbBot, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any KolbBot host.
