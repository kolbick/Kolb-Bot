...
If `delivery` is omitted for isolated jobs, KolbBot defaults to `announce`.
...
- Run history: `~/.kolb-bot/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
- KolbBot applies exponential retry backoff for recurring jobs after consecutive errors:
