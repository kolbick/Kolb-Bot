---
summary: "Chrome extension: let KolbBot drive your existing Chrome tab"
read_when:
...
The KolbBot Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate kolb-bot-managed Chrome profile.
...
KolbBot then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the KolbBot release (npm package) as static files. There is no separate “build” step.

After upgrading KolbBot:

- Re-run `kolb-bot browser extension install` to refresh the installed files under your KolbBot state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

KolbBot ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want KolbBot to control.
...
- `ON`: attached; KolbBot can drive that tab.
...
The CLI intentionally does **not** print a `node_modules` path. Always run `kolb-bot browser extension install` first to copy the extension to a stable location under your KolbBot state directory.
