---
summary: "Agent tool surface for KolbBot (browser, canvas, nodes, message, cron) replacing legacy `kolb-bot-*` skills"
read_when:
...
KolbBot exposes **first-class agent tools** for browser, canvas, nodes, and cron.
...
- If `tools.allow` only references unknown or unloaded plugin tool names, KolbBot logs a warning and ignores the allowlist so core tools stay available.
...
- Requires a Brave API key (recommended: `kolb-bot configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated KolbBot-managed browser.
