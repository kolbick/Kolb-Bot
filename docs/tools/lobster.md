---
title: Lobster
summary: "Typed workflow runtime for KolbBot with resumable approval gates."
description: Typed workflow runtime for KolbBot — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets KolbBot run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: KolbBot runs one Lobster tool call and gets a structured result.
...
KolbBot launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In KolbBot tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the KolbBot Gateway (see the [Lobster repo](https://github.com/kolb-bot/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), KolbBot keeps core tools enabled. To restrict core
...
- **No secrets** — Lobster doesn't manage OAuth; it calls KolbBot tools that do.
