...
If you want to add new capabilities to your KolbBot agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
By default, the CLI installs skills into `./skills` under your current working directory. If a KolbBot workspace is configured, `clawhub` falls back to that workspace unless you override `--workdir` (or `CLAWHUB_WORKDIR`). KolbBot loads workspace skills from `<workspace>/skills` and will pick them up in the **next** session. If you already use `~/.kolb-bot/skills` or bundled skills, workspace skills take precedence.
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/kolb-bot/skills` and `~/.kolb-bot/skills`). This is designed to find older skill installs without extra flags.
