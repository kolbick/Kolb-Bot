...
title: "Browser (KolbBot-managed)"
---

# Browser (kolb-bot-managed)

KolbBot can run a **dedicated Chrome/Brave/Edge/Chromium profile** that the agent controls.
...
- `chrome`: extension relay to your **system browser** (requires the KolbBot
...
Browser settings live in `~/.kolb-bot/kolb-bot.json`.
...
- Default profile is `chrome` (extension relay). Use `defaultProfile: "kolb-bot"` for the managed browser.
...
KolbBot uses it automatically. Set `browser.executablePath` to override
auto-detection:
...
- **Remote CDP:** set `browser.profiles.<name>.cdpUrl` (or `browser.cdpUrl`) to
  attach to a remote Chromium-based browser. In this case, KolbBot will not launch a local browser.
...
KolbBot preserves the auth when calling `/json/*` endpoints and when connecting
to the CDP WebSocket. Prefer environment variables or secrets managers for
tokens instead of committing them to config files.

## Node browser proxy (zero-config default)

If you run a **node host** on the machine that has your browser, KolbBot can
...
CDP endpoints over HTTPS. You can point a KolbBot browser profile at a
...
KolbBot supports multiple named profiles (routing configs). Profiles can be:
...
KolbBot can also drive **your existing Chrome tabs** (no separate “kolb-bot” Chrome instance) via a local CDP relay + a Chrome extension.
...
- You click the **KolbBot Browser Relay** extension icon on a tab to attach (it does not auto-attach).
...
When launching locally, KolbBot picks the first available:
...
KolbBot supports two “snapshot” styles:
