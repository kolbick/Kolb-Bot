...
- The token is `gateway.auth.token` from the gateway config (`~/.kolb-bot/kolb-bot.json` on the gateway host).
- `kolb-bot node run` reads `KOLB_BOT_GATEWAY_TOKEN` for auth.
...
KolbBot can run a **headless node host** (no UI) that connects to the Gateway
...
- On macOS, the headless node host prefers the companion app exec host when reachable and falls
  back to local execution if the app is unavailable. Set `KOLB_BOT_NODE_EXEC_HOST=app` to require
  the app, or `KOLB_BOT_NODE_EXEC_FALLBACK=0` to disable fallback.
