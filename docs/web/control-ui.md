...
- Config: view/edit `~/.kolb-bot/kolb-bot.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. KolbBot
...
KolbBot **blocks** Control UI connections without device identity.
