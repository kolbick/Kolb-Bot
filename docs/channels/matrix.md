...
Matrix is an open, decentralized messaging protocol. KolbBot connects as a Matrix **user**
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: KolbBot calls the same
...
- On first connection, KolbBot requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  KolbBot logs a warning.
...
- On startup, KolbBot resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
