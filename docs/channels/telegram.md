...
KolbBot can stream partial replies in Telegram DMs using `sendMessageDraft`.
...
- If Telegram rejects the HTML payload, KolbBot retries the same message as plain text.

## Commands (native + custom)

KolbBot registers native commands (like `/status`, `/reset`, `/model`) with Telegram’s bot menu on startup.
...
- Custom commands are **menu entries only**; KolbBot does not implement them unless you handle them elsewhere.
...
**Privacy note:** `@userinfobot` is a third-party bot. If you prefer, add the bot to the group, send a message, and use `kolb-bot logs --follow` to read `chat.id`, or use the Bot API `getUpdates`.
...
- A group is upgraded to a supergroup and Telegram emits `migrate_to_chat_id` (chat ID changes). KolbBot can migrate `channels.telegram.groups` automatically.
...
Telegram forum topics include a `message_thread_id` per message. KolbBot:
...
Private chats can include `message_thread_id` in some edge cases. KolbBot keeps the DM session key unchanged, but still uses the thread id for replies/draft streaming when it is present.
...
2. Run `kolb-bot logs --follow` and look for `from.id`.
...
KolbBot defaults to audio files for backward compatibility.
...
KolbBot supports receiving and sending Telegram stickers with intelligent caching.

### Receiving stickers

When a user sends a sticker, KolbBot handles it based on the sticker type:
...
Stickers are processed through the AI's vision capabilities to generate descriptions. Since the same stickers are often sent repeatedly, KolbBot caches these descriptions to avoid redundant API calls.
...
**Cache location:** `~/.kolb-bot/telegram/sticker-cache.json`
...
KolbBot uses Bot API `sendMessageDraft` (not real messages) and then sends the
...
**How reactions work:**
Telegram reactions arrive as **separate `message_reaction` events**, not as properties in message payloads. When a user adds a reaction, KolbBot:
...
- Telegram bots must explicitly request `message_reaction` in `allowed_updates` (configured automatically by KolbBot)
...
- Example: `kolb-bot message send --channel telegram --target 123456789 --message "hi"`.
...
- `kolb-bot channels status` shows a warning when config expects unmentioned group messages.
- `kolb-bot channels status --probe` can additionally check membership for explicit numeric group IDs (it can’t audit wildcard `"*"` rules).
...
- Check gateway logs: `kolb-bot logs --follow` (look for "skipping group message")
...
- Upgrade to a KolbBot build that normalizes abort signals, or run the gateway on Node 20 until you can upgrade.
