...
KolbBot will offer the local install path automatically.
...
3. Configure KolbBot with those credentials.
...
- Talk to KolbBot via Teams DMs, group chats, or channels.
...
- On startup, KolbBot resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.kolb-bot/kolb-bot.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring KolbBot, you need to create an Azure Bot resource.
...
```json
{
  ...
  "description": { "short": "KolbBot in Teams", "full": "KolbBot in Teams" },
...
KolbBot handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, KolbBot only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/KolbBotShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

KolbBot sends Teams polls as Adaptive Cards (there is no native Teams poll API).
