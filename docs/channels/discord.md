...
- Talk to KolbBot via Discord DMs or guild channels.
...
3. Configure KolbBot with `channels.discord.token` (or `DISCORD_BOT_TOKEN` as a fallback).
...
7. Direct chats: secure by default via `channels.discord.dm.policy` (default: `"pairing"`). Unknown senders get a pairing code (expires after 1 hour); approve via `kolb-bot pairing approve discord <code>`.
...
This is the “Discord Developer Portal” setup for running KolbBot in a server (guild) channel like `#help`.
...
### 2) Enable the gateway intents KolbBot needs
...
Discord uses numeric ids everywhere; KolbBot config prefers ids.
...
- Owner hint: when a per-guild or per-channel `users` allowlist matches the sender, KolbBot treats that sender as the owner in the system prompt. For a global owner across channels, set `commands.ownerAllowFrom`.
...
- First: run `kolb-bot doctor` and `kolb-bot channels status --probe` (actionable warnings + quick audits).
...
When enabled, KolbBot uses the member identity for allowlists and labels the
...
- On startup, KolbBot resolves channel/user names in allowlists to IDs (when the bot can search members)
  and logs the mapping; unresolved entries are kept as typed.

Native command notes:

- The registered commands mirror KolbBot’s chat commands.
...
- Slash commands may still be visible in Discord UI to users who aren’t allowlisted; KolbBot enforces allowlists on execution and replies “not authorized”.
...
- If the bot is stuck or rate limited, restart the gateway (`kolb-bot gateway --force`) after confirming no other processes own the Discord session.
