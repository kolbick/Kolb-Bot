...
3. Set tokens for KolbBot and start the gateway.
...
7. Slash Commands → create `/kolb-bot` if you use `channels.slack.slashCommand`. If you enable native commands, add one slash command per built-in command (same names as `/help`). Native defaults to off for Slack unless you set `channels.slack.commands.native: true` (global `commands.native` is `"auto"` which leaves Slack off).
...
KolbBot can use a Slack user token (`xoxp-...`) for read operations (history,
...
- Write operations (send/edit/delete messages, add/remove reactions, pin/unpin,
  ...
  no bot token is available, KolbBot falls back to the user token.
...
```json
{
  "display_information": {
    "name": "KolbBot",
    "description": "Slack connector for KolbBot"
  },
  "features": {
    "bot_user": {
      "display_name": "KolbBot",
      "always_online": false
    },
    "app_home": {
    ...
    "slash_commands": [
      {
        "command": "/kolb-bot",
        "description": "Send a message to KolbBot",
        "should_escape": false
      }
    ]
  },
...
By default, KolbBot replies in the main channel. Use `channels.slack.replyToMode` to control automatic threading:
...
- If Slack doesn’t provide `channel_type`, KolbBot infers it from the channel ID prefix (`D`, `C`, `G`) and defaults to `channel` to keep session keys stable.
...
- Approve via: `kolb-bot pairing approve slack <code>`.
...
- On startup, KolbBot resolves channel/user names in allowlists to IDs (when tokens allow)
