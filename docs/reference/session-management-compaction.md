...
This document explains how KolbBot manages sessions end-to-end:
...
KolbBot is designed around a single **Gateway process** that owns session state.
...
KolbBot persists sessions in two layers:
...
KolbBot resolves these via `src/config/sessions.ts`.
...
KolbBot intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (KolbBot consumes the events, but Pi decides when to compact).
...
KolbBot also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, KolbBot bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, KolbBot leaves it alone.
...
KolbBot supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- KolbBot strips/suppresses this in the delivery layer.

As of `2026.1.10`, KolbBot also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
KolbBot uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but KolbBot’s
