...
KolbBot integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- KolbBot owns lifecycle when `channels.signal.autoStart=true`.
...
- KolbBot spawns `imsg rpc` as a child process (legacy iMessage integration).
