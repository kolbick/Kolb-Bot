---
summary: "Advanced setup and development workflows for KolbBot"
read_when:
...
- **Tailoring lives outside the repo:** `~/.kolb-bot/workspace` (workspace) + `~/.kolb-bot/kolb-bot.json` (config).
...
- **Config:** `~/.kolb-bot/kolb-bot.json` (JSON/JSON5-ish)
...
1. Install + launch **KolbBot.app** (menu bar).
...
- [KolbBot assistant setup](/start/kolb-bot)
