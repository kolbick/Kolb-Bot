...
read_when:
  - You want to understand what “context” means in KolbBot
...
“Context” is **everything KolbBot sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (KolbBot-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How KolbBot builds the system prompt

The system prompt is **KolbBot-owned** and rebuilt each run. It includes:
...
By default, KolbBot injects a fixed set of workspace files (if present):
