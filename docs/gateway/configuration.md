---
summary: "All configuration options for ~/.kolb-bot/kolb-bot.json with examples"
read_when:
...
KolbBot reads an optional **JSON5** config from `~/.kolb-bot/kolb-bot.json` (comments + trailing commas allowed).

If the file is missing, KolbBot uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/.kolb-bot/workspace`). You usually only need a config to:
...
KolbBot only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:

- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `kolb-bot doctor`, `kolb-bot logs`, `kolb-bot health`, `kolb-bot status`, `kolb-bot service`, `kolb-bot help`).
- Run `kolb-bot doctor` to see the exact issues.
- Run `kolb-bot doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `kolb-bot config set`. Keep a backup of `~/.kolb-bot/kolb-bot.json`.
...
{
  agents: {
    defaults: { workspace: "~/.kolb-bot/workspace" },
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["@kolb-bot", "reisponde"] },
      },
    ],
  },
  channels: {
...
```json5
// ~/.kolb-bot/agents.json5
{
  defaults: { sandbox: { mode: "all", scope: "session" } },
  list: [{ id: "main", workspace: "~/.kolb-bot/workspace" }],
}
```
...
{ "$include": "/etc/kolb-bot/base.json5" } // absolute
...
```json5
// ~/.kolb-bot/clients/mueller/agents.json5
[
...
```json5
// ~/.kolb-bot/clients/mueller/broadcast.json5
{
...
KolbBot reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, KolbBot runs your login shell and imports only the missing expected keys (never overrides).
...
KolbBot stores **per-agent** auth profiles (OAuth + API keys) in:

- `<agentDir>/auth-profiles.json` (default: `~/.kolb-bot/agents/<agentId>/agent/auth-profiles.json`)
...
- `~/.kolb-bot/agent/*` (migrated by `kolb-bot doctor` into `~/.kolb-bot/agents/<defaultAgentId>/agent/*`)

Overrides:

- OAuth dir (legacy import only): `KOLB_BOT_OAUTH_DIR`
- Agent dir (default agent root override): `KOLB_BOT_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, KolbBot imports `oauth.json` entries into `auth-profiles.json`.
...
If set, KolbBot derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/kolb-bot/kolb-bot-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/kolb-bot/kolb-bot.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/kolb-bot/kolb-bot.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
...
- The legacy single-account Baileys auth dir is migrated by `kolb-bot doctor` into `whatsapp/default`.
    ...
    list: [
      {
        ...
        groupChat: {
          ...
          mentionPatterns: ["reisponde", "@kolb-bot"],
        },
      },
    ],
  },
...
- `agents.list[]`: per-agent overrides.
  ...
  - `workspace`: default `~/.kolb-bot/workspace-<agentId>` (for `main`, falls back to `agents.defaults.workspace`).
  - `agentDir`: default `~/.kolb-bot/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `kolb-bot.json`).
...
KolbBot starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
  ...
  channels: {
    discord: {
      ...
      dm: {
        ...
        groupChannels: ["kolb-bot-dm"], // optional group DM allowlist
      },
      guilds: {
        "123456789012345678": {
...
KolbBot starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `kolb-bot plugins install @kolb-bot/mattermost` (or `./extensions/mattermost` from a git checkout).
...
KolbBot starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
KolbBot spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, KolbBot
...
When a file exceeds this limit, KolbBot logs a warning and injects a truncated
...
message envelopes). If unset, KolbBot uses the host timezone at runtime.
...
`[kolb-bot]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[kolb-bot]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[kolb-bot]"`, KolbBot will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, KolbBot generates audio
...
`voiceAliases` lets Talk directives use friendly names (e.g. `"voice":"KolbBot"`).
...
KolbBot also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, KolbBot currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `kolb-bot configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:kolb-bot`: all built-in KolbBot tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.kolb-bot/sandboxes`
...
KolbBot uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.kolb-bot/agents/<agentId>/agent/models.json` or by defining the same schema inside your
KolbBot config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, KolbBot writes/merges a `models.json` into
`~/.kolb-bot/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. KolbBot uses
...
- Shortcut: `kolb-bot onboard --auth-choice opencode-zen`.
...
Shortcut: `kolb-bot onboard --auth-choice zai-api-key`.
...
- Set `MOONSHOT_API_KEY` in the environment or use `kolb-bot onboard --auth-choice moonshot-api-key`.
- Model ref: `moonshot/kimi-k2.5`.
- For the China endpoint, either:
  - Run `kolb-bot onboard --auth-choice moonshot-api-key-cn` (wizard will set `https://api.moonshot.cn/v1`), or
...
- Set `KIMI_API_KEY` in the environment or use `kolb-bot onboard --auth-choice kimi-code-api-key`.
...
- Set `SYNTHETIC_API_KEY` or use `kolb-bot onboard --auth-choice synthetic-api-key`.
...
- Set `MINIMAX_API_KEY` environment variable or use `kolb-bot onboard --auth-choice minimax-api`.
...
- Override the agent config root with `KOLB_BOT_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.kolb-bot/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.kolb-bot/agents/<agentId>/sessions/sessions.json
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, KolbBot stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.kolb-bot/skills` (workspace skills
...
from `~/.kolb-bot/extensions`, `<workspace>/.kolb-bot/extensions`, plus any
...
### `browser` (kolb-bot-managed browser)

KolbBot can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for kolb-bot and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (KolbBot.app menubar, or `kolb-bot gateway`).
...
- Examples: `"/ui"`, `"/kolb-bot"`, `"/apps/kolb-bot"`.
...
- When a connection comes from one of these IPs, KolbBot uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:

- `kolb-bot gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `KOLB_BOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `KOLB_BOT_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. KolbBot
...
- KolbBot.app watches `~/.kolb-bot/kolb-bot.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.kolb-bot/kolb-bot.json` (or `KOLB_BOT_CONFIG_PATH`) and applies changes automatically.
...
- `~/.kolb-bot/kolb-bot.json` (or `KOLB_BOT_CONFIG_PATH`)
...
- `KOLB_BOT_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):

- `kolb-bot --dev …` → uses `~/.kolb-bot-dev` + shifts ports from base `19001`
- `kolb-bot --profile <name> …` → uses `~/.kolb-bot-<name>` (port via config/env/flags)
...
Gmail helper config (used by `kolb-bot webhooks gmail setup` / `run`):
...
- Set `KOLB_BOT_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, KolbBot defaults `serve.path` to `/` so
...
Default root: `~/.kolb-bot/workspace/canvas`  
Default port: `18793` (chosen to avoid the kolb-bot browser CDP port `18792`)  
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__kolb-bot__/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__kolb-bot__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `KOLB_BOT_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.kolb-bot/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.kolb-bot/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.kolb-bot/bridge/tls/bridge-key.pem"
    },
  },
}
```
...
Controls LAN mDNS discovery broadcasts (`_kolb-bot-gw._tcp`).

- `minimal` (default): omit `cliPath` + `sshPort` from TXT records
- `full`: include `cliPath` + `sshPort` in TXT records
- `off`: disable mDNS broadcasts entirely
- Hostname: defaults to `kolb-bot` (advertises `kolb-bot.local`). Override with `KOLB_BOT_MDNS_HOSTNAME`.
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_kolb-bot-gw._tcp` under `~/.kolb-bot/dns/` using the configured discovery domain (example: `kolb-bot.internal.`).
