...
- Replaces the legacy `gateway` command. CLI entry point: `kolb-bot gateway`.
...
- Config hot reload watches `~/.kolb-bot/kolb-bot.json` (or `KOLB_BOT_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__kolb-bot__/canvas/` from `~/.kolb-bot/workspace/canvas`. Disable with `canvasHost.enabled=false` or `KOLB_BOT_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `KOLB_BOT_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `KOLB_BOT_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `bot.molt.<profile>` (legacy `com.kolb-bot.*` may still exist)
- Linux: `kolb-bot-gateway-<profile>.service`
- Windows: `KolbBot Gateway (<profile>)`

Install metadata is embedded in the service config:

- `KOLB_BOT_SERVICE_MARKER=kolb-bot`
...
- `KOLB_BOT_GATEWAY_PORT=19001` (Gateway WS + HTTP)
- browser control service port = `19003` (derived: `gateway.port+2`, loopback only)
- `canvasHost.port=19005` (derived: `gateway.port+4`)
- `agents.defaults.workspace` default becomes `~/.kolb-bot/workspace-dev` when you run `setup`/`onboard` under `--dev`.

Derived ports (rules of thumb):

- Base port = `gateway.port` (or `KOLB_BOT_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `KOLB_BOT_CANVAS_HOST_PORT` / config override)
...
- `health` — full health snapshot (same shape as `kolb-bot health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `kolb-bot`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `kolb-bot gateway install` writes `~/Library/LaunchAgents/bot.molt.gateway.plist`
    (or `bot.molt.<profile>.plist`; legacy `com.kolb-bot.*` is cleaned up).
  - `kolb-bot doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are KolbBot profile services.
  ...
  - Cleanup: `kolb-bot gateway uninstall` (current service) and `kolb-bot doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `kolb-bot gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:

- KolbBot.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `bot.molt.gateway` (or `bot.molt.<profile>`; legacy `com.kolb-bot.*` labels still unload cleanly).
- To stop it cleanly, use `kolb-bot gateway stop` (or `launchctl bootout gui/$UID/bot.molt.gateway`).
- To restart, use `kolb-bot gateway restart` (or `launchctl kickstart -k gui/$UID/bot.molt.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `kolb-bot gateway install` first.
  - Replace the label with `bot.molt.<profile>` when running a named profile.

## Supervision (systemd user unit)

KolbBot installs a **systemd user service** by default on Linux/WSL2. We
...
`kolb-bot gateway install` writes the user unit. `kolb-bot doctor` audits the
...
```
[Unit]
Description=KolbBot Gateway (profile: <profile>, v<version>)
...
Restart=always
RestartSec=5
Environment=KOLB_BOT_GATEWAY_TOKEN=
...
Create `/etc/systemd/system/kolb-bot-gateway[-<profile>].service` (copy the unit above,
...
- `kolb-bot gateway health|status` — request health/status over the Gateway WS.
- `kolb-bot message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `kolb-bot agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `kolb-bot gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `kolb-bot gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance

- Retire uses of `kolb-bot gateway` and the legacy TCP control port.
