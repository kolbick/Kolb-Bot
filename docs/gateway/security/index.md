...
- Tighten local perms (`~/.kolb-bot` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

KolbBot is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, KolbBot also attempts a best-effort live Gateway probe.
...
- **Pairing allowlists**: `~/.kolb-bot/credentials/<channel>-allowFrom.json`
...
KolbBot stores session transcripts on disk under `~/.kolb-bot/agents/<agentId>/sessions/*.jsonl`.
...
KolbBot can refresh the skills list mid-session:
...
- If you install plugins from npm (`kolb-bot plugins install <npm-spec>`), treat it like running untrusted code:
  ...
  - KolbBot uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
By default, KolbBot routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
KolbBot has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.kolb-bot/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
4. **Re-run audit**
   - `kolb-bot security audit --deep` and confirm the report is clean.

## Lessons Learned (The Hard Way)

### The `find ~` Incident 🦞

On Day 1, a friendly tester asked KolbBot to run `find ~` and share the output. KolbBot happily dumped the entire home directory structure to a group chat.
...
- `~/.kolb-bot/kolb-bot.json`: `600` (user read/write only)
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), KolbBot
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. KolbBot verifies the identity by resolving the
...
- KolbBot will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only KolbBot clients can connect.
...
1. Check Gateway logs: `/tmp/kolb-bot/kolb-bot-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.kolb-bot/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + KolbBot version
