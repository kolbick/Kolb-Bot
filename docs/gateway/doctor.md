...
`kolb-bot doctor` is the repair + migration tool for KolbBot. It fixes stale
...
- Rewrite `~/.kolb-bot/kolb-bot.json` with the updated schema.
...
- **Config file permissions**: warns if `~/.kolb-bot/kolb-bot.json` is
...
offers to remove them and install the KolbBot service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named KolbBot gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `kolb-bot doctor --generate-gateway-token` to force token
