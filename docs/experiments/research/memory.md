...
read_when:
  ...
  - Deciding: standalone CLI vs deep KolbBot integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.kolb-bot/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Recommendation: **deep integration in KolbBot**, but keep a separable core library.

### Why integrate into KolbBot?

- KolbBot already knows:
