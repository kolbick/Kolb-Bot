...
- **会话存储**（聊天历史 + 路由状态）位于 `~/.kolb-bot/agents/<agentId>/sessions` 下。
...
Skills 通过每个工作区的 `skills/` 文件夹实现每智能体独立，共享的 Skills 可从 `~/.kolb-bot/skills` 获取。参见 [Skills：每智能体 vs 共享](/tools/skills#per-agent-vs-shared-skills)。
...
- 状态目录：`~/.kolb-bot`（或 `KOLB_BOT_STATE_DIR`）
- 工作区：`~/.kolb-bot/workspace`（或 `~/.kolb-bot/workspace-<agentId>`）
- 智能体目录：`~/.kolb-bot/agents/<agentId>/agent`（或 `agents.list[].agentDir`）
- 会话：`~/.kolb-bot/agents/<agentId>/sessions`

### 单智能体模式（默认）

如果你什么都不做，KolbBot 运行单个智能体：

- `agentId` 默认为 **`main`**。
- 会话键为 `agent:main:<mainKey>`。
- 工作区默认为 `~/.kolb-bot/workspace`（或当设置了 `KOLB_BOT_PROFILE` 时为 `~/.kolb-bot/workspace-<profile>`）。
- 状态默认为 `~/.kolb-bot/agents/main/agent`。
...
```js
{
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // 可选覆盖。默认：~/.kolb-bot/credentials/whatsapp/personal
