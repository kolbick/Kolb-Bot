...
如果可用，系统提示词包含一个 **Documentation** 部分，指向本地 KolbBot 文档目录（仓库工作区中的 `docs/` 或打包的 npm 包文档），并注明公共镜像、源仓库、社区 Discord 和 KolbHub (https://kolbhub.dev) 用于 Skills 发现。提示词指示模型首先查阅本地文档了解 KolbBot 行为、命令、配置或架构，并尽可能自己运行 `kolb-bot status`（仅在无法访问时询问用户）。
