...
read_when:
  - 启动新的 KolbBot 智能体会话
  - 启用或审计默认 Skills
summary: 个人助手设置的默认 KolbBot 智能体指令和 Skills 列表
x-i18n:
...
# AGENTS.md — KolbBot 个人助手（默认）

## 首次运行（推荐）

KolbBot 为智能体使用专用的工作区目录。默认：`~/.kolb-bot/workspace`（可通过 `agents.defaults.workspace` 配置）。
...
- macOS 应用管理权限（屏幕录制、通知、麦克风）并通过其内置二进制文件暴露 `kolb-bot` CLI。
...
- 脚本编写优先使用 `kolb-bot` CLI；mac 应用处理权限。
...
- 对于浏览器驱动的验证，使用带有 KolbBot 管理的 Chrome 配置文件的 `kolb-bot browser`（tabs/status/screenshot）。
- 对于 DOM 检查，使用 `kolb-bot browser eval|query|dom|snapshot`（需要机器输出时使用 `--json`/`--out`）。
- 对于交互，使用 `kolb-bot browser click|type|hover|drag|select|upload|press|wait|navigate|back|evaluate|run`（click/type 需要 snapshot 引用；CSS 选择器使用 `evaluate`）。
