...
summary: 设置指南：在保持最新的同时保持你的 KolbBot 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.kolb-bot/workspace`（工作区）+ `~/.kolb-bot/kolb-bot.json`（配置）。
...
- **配置：** `~/.kolb-bot/kolb-bot.json`（JSON/JSON5 格式）
- **工作区：** `~/.kolb-bot/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm kolb-bot setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **KolbBot.app**（菜单栏）。
...
- 运行 `kolb-bot setup`，然后 `kolb-bot channels login`，然后手动启动 Gateway 网关（`kolb-bot gateway`）。
...
- **配对允许列表**：`~/.kolb-bot/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.kolb-bot/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.kolb-bot/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.kolb-bot/workspace` 和 `~/.kolb-bot/` 保持为"你的东西"；不要将个人提示/配置放入 `kolb-bot` 仓库。
