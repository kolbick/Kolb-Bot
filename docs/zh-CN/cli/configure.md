...
summary: "`kolb-bot configure` 的 CLI 参考（交互式配置提示）"
title: configure
x-i18n:
...
提示：不带子命令的 `kolb-bot config` 会打开相同的向导。使用 `kolb-bot config get|set|unset` 进行非交互式编辑。
...
```bash
kolb-bot configure
kolb-bot configure --section models --section channels
```
