...
```bash
kolb-bot plugins install ./extensions/feishu
```
...
如果您刚安装完 KolbBot，可以直接运行向导，根据提示添加飞书：
...
- `kolb-bot gateway status` - 查看网关运行状态
- `kolb-bot logs --follow` - 查看实时日志
...
- `kolb-bot gateway status` - 查看网关运行状态
- `kolb-bot gateway restart` - 重启网关以应用新配置
- `kolb-bot logs --follow` - 查看实时日志
...
1. 运行 `kolb-bot channels add` 添加了 Feishu 渠道
2. 网关处于启动状态（可通过 `kolb-bot gateway status` 检查状态）
...
```bash
kolb-bot pairing approve feishu <配对码>
```
...
- **批准配对**：
  ```bash
  kolb-bot pairing list feishu      # 查看待审批列表
  kolb-bot pairing approve feishu <CODE>  # 批准
  ```
...
1. 启动网关并在群组中 @机器人发消息
2. 运行 `kolb-bot logs --follow` 查看日志中的 `chat_id`
...
1. 启动网关并给机器人发消息
2. 运行 `kolb-bot logs --follow` 查看日志中的 `open_id`
...
| -------------------------- | ----------------- |
| `kolb-bot gateway status`  | 查看网关运行状态  |
| `kolb-bot gateway install` | 安装/启动网关服务 |
| `kolb-bot gateway stop`    | 停止网关服务      |
| `kolb-bot gateway restart` | 重启网关服务      |
| `kolb-bot logs --follow`   | 实时查看日志输出  |
...
3. 检查是否选择了 **长连接** 模式
4. 检查应用权限是否完整
5. 检查网关是否正在运行：`kolb-bot gateway status`
6. 查看实时日志：`kolb-bot logs --follow`
