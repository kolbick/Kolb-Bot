...
| 命令                               | 它告诉你什么                                                                          | 何时使用                              |
| ---------------------------------- | ------------------------------------------------------------------------------------- | ------------------------------------- |
| `kolb-bot status`                  | 本地摘要：操作系统 + 更新、Gateway 网关可达性/模式、服务、智能体/会话、提供商配置状态 | 首次检查，快速概览                    |
| `kolb-bot status --all`            | 完整本地诊断（只读、可粘贴、相对安全）包括日志尾部                                    | 当你需要分享调试报告时                |
| `kolb-bot status --deep`           | 运行 Gateway 网关健康检查（包括提供商探测；需要可达的 Gateway 网关）                  | 当"已配置"不意味着"正常工作"时        |
| `kolb-bot gateway probe`           | Gateway 网关发现 + 可达性（本地 + 远程目标）                                          | 当你怀疑正在探测错误的 Gateway 网关时 |
| `kolb-bot channels status --probe` | 向运行中的 Gateway 网关查询渠道状态（并可选探测）                                     | 当 Gateway 网关可达但渠道异常时       |
| `kolb-bot gateway status`          | 监管程序状态（launchd/systemd/schtasks）、运行时 PID/退出、最后的 Gateway 网关错误    | 当服务"看起来已加载"但没有运行时      |
| `kolb-bot logs --follow`           | 实时日志（运行时问题的最佳信号）                                                      | 当你需要实际的故障原因时              |

**分享输出：** 优先使用 `kolb-bot status --all`（它会隐藏令牌）。如果你粘贴 `kolb-bot status`，考虑先设置 `KOLB_BOT_SHOW_SECRETS=0`（令牌预览）。
...
- 优先：`kolb-bot logs --follow`
- 文件日志（始终）：`/tmp/kolb-bot/kolb-bot-YYYY-MM-DD.log`（或你配置的 `logging.file`）
- macOS LaunchAgent（如果已安装）：`$KOLB_BOT_STATE_DIR/logs/gateway.log` 和 `gateway.err.log`
- Linux systemd（如果已安装）：`journalctl --user -u kolb-bot-gateway[-<profile>].service -n 200 --no-pager`
...
- 或直接设置：
  ```bash
  kolb-bot config set gateway.mode local
  ```
...
- 设置远程 URL 并保持 `gateway.mode=remote`：
  ```bash
  kolb-bot config set gateway.mode remote
  kolb-bot config set gateway.remote.url "wss://gateway.example.com"
  ```

**仅临时/开发使用：** 传递 `--allow-unconfigured` 以在没有
`gateway.mode=local` 的情况下启动 Gateway 网关。

**还没有配置文件？** 运行 `kolb-bot setup` 创建初始配置，然后重新运行
Gateway 网关。
...
变量如 `DISPLAY` 应该放在 `~/.kolb-bot/.env` 中（由 Gateway 网关早期加载）。
...
服务是用 Bun 或版本管理的 Node 路径安装的，运行 `kolb-bot doctor`
迁移到系统 Node 安装。
...
- 或将密钥烘焙到你的自定义沙箱镜像中
- 然后运行 `kolb-bot sandbox recreate --agent <id>`（或 `--all`）
...
- `gateway.mode` 必须为 `local` 才能运行 `kolb-bot gateway` 和服务。
- 如果你设置了 `gateway.mode=remote`，**CLI 默认**使用远程 URL。服务可能仍在本地运行，但你的 CLI 可能在探测错误的位置。使用 `kolb-bot gateway status` 查看服务解析的端口 + 探测目标（或传递 `--url`）。
- `kolb-bot gateway status` 和 `kolb-bot doctor` 在服务看起来正在运行但端口关闭时会显示日志中的**最后 Gateway 网关错误**。
- 非本地回环绑定（`lan`/`tailnet`/`custom`，或本地回环不可用时的 `auto`）需要认证：
  `gateway.auth.token`（或 `KOLB_BOT_GATEWAY_TOKEN`）。
- `gateway.remote.token` 仅用于远程 CLI 调用；它**不**启用本地认证。
- `gateway.token` 被忽略；使用 `gateway.auth.token`。

**如果 `kolb-bot gateway status` 显示配置不匹配**

- `Config (cli): ...` 和 `Config (service): ...` 通常应该匹配。
- 如果不匹配，你几乎肯定是在编辑一个配置而服务运行的是另一个。
- 修复：从你希望服务使用的相同 `--profile` / `KOLB_BOT_STATE_DIR` 重新运行 `kolb-bot gateway install --force`。

**如果 `kolb-bot gateway status` 报告服务配置问题**

- 监管程序配置（launchd/systemd/schtasks）缺少当前默认值。
- 修复：运行 `kolb-bot doctor` 更新它（或 `kolb-bot gateway install --force` 完全重写）。
...
- 修复：设置 `gateway.auth.mode` + `gateway.auth.token`（或导出 `KOLB_BOT_GATEWAY_TOKEN`）并重启服务。

**如果 `kolb-bot gateway status` 显示 `bind=tailnet` 但未找到 tailnet 接口**
...
症状：`pwd` 或文件工具显示 `~/.kolb-bot/sandboxes/...` 即使你
期望的是主机工作区。
...
KolbBot 有意拒绝**较旧/不安全的模型**（尤其是那些更容易受到提示词注入攻击的模型）。如果你看到此错误，该模型名称已不再支持。

**修复：**

- 为提供商选择**最新**模型并更新你的配置或模型别名。
- 如果你不确定哪些模型可用，运行 `kolb-bot models list` 或
  `kolb-bot models scan` 并选择一个支持的模型。
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${KOLB_BOT_CONFIG_PATH:-$HOME/.kolb-bot/kolb-bot.json}"
```
...
# 或者如果你想快速过滤：
tail -f "$(ls -t /tmp/kolb-bot/kolb-bot-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
kolb-bot logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.kolb-bot/agents/<agentId>/sessions/
```
...
# 查看最近的连接事件
kolb-bot logs --limit 200 | grep "connection\\|disconnect\\|logout"
```

**修复：** 通常在 Gateway 网关运行后会自动重连。如果卡住，重启 Gateway 网关进程（无论你如何监管它），或使用详细输出手动运行：

```bash
kolb-bot gateway --verbose
```

如果你已登出/取消关联：

```bash
kolb-bot channels logout
trash "${KOLB_BOT_STATE_DIR:-$HOME/.kolb-bot}/credentials" # 如果 logout 无法完全清除所有内容
kolb-bot channels login --verbose       # 重新扫描二维码
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/kolb-bot/kolb-bot-*.log | head -1)" | tail -20
```
...
当配置包含未知键、格式错误的值或无效类型时，KolbBot 现在拒绝启动。
这是为了安全而故意设计的。
...
- `kolb-bot doctor` 报告每个无效条目。
- `kolb-bot doctor --fix` 应用迁移/修复并重写配置。
- 诊断命令如 `kolb-bot logs`、`kolb-bot health`、`kolb-bot status`、`kolb-bot gateway status` 和 `kolb-bot gateway probe` 即使配置无效也能运行。

### "All models failed" — 我应该首先检查什么？

- **凭证**存在于正在尝试的提供商（认证配置文件 + 环境变量）。
- **模型路由**：确认 `agents.defaults.model.primary` 和回退是你可以访问的模型。
- `/tmp/kolb-bot/…` 中的 **Gateway 网关日志**以获取确切的提供商错误。
- **模型状态**：使用 `/model status`（聊天）或 `kolb-bot models status`（CLI）。
...
5. 运行 `kolb-bot channels status --probe` 获取审计提示。
...
端点接受 JSON Schema 的严格子集。KolbBot 在当前 `main` 中清理/规范化工具
...
如果重置不起作用，在 [`scripts/package-mac-app.sh`](https://github.com/kolb-bot/kolb-bot/blob/main/scripts/package-mac-app.sh) 中更改 `BUNDLE_ID`（例如，添加 `.test` 后缀）并重新构建。这会强制 macOS 将其视为新应用。
...
# 或：launchctl bootout gui/$UID/bot.molt.gateway（用 bot.molt.<profile> 替换；旧版 com.kolb-bot.* 仍然有效）
```
...
```bash
# 在配置中打开跟踪日志：
#   ${KOLB_BOT_CONFIG_PATH:-$HOME/.kolb-bot/kolb-bot.json} -> { logging: { level: "trace" } }
#
...
kolb-bot logs --follow
# 如果 RPC 宕机的备用方案
tail -20 /tmp/kolb-bot/kolb-bot-*.log
```
...
trash "${KOLB_BOT_STATE_DIR:-$HOME/.kolb-bot}"
kolb-bot channels login         # 重新配对 WhatsApp
kolb-bot gateway restart           # 或：kolb-bot gateway
```
...
1. 首先检查日志：`/tmp/kolb-bot/`（默认：`kolb-bot-YYYY-MM-DD.log`，或你配置的 `logging.file`）
2. 在 GitHub 上搜索现有问题
3. 提交新问题时包含：
   - KolbBot 版本
   - 相关日志片段
