---
read_when:
  - 在 DigitalOcean 上设置 KolbBot
  - 寻找便宜的 VPS 托管来运行 KolbBot
summary: 在 DigitalOcean 上运行 KolbBot（简单的付费 VPS 选项）
title: DigitalOcean
x-i18n:
...
# 在 DigitalOcean 上运行 KolbBot

## 目标

以 **$6/月**（或使用预留定价 $4/月）在 DigitalOcean 上运行持久的 KolbBot Gateway 网关。
...
- `~/.kolb-bot/` — 配置、凭证、会话数据
- `~/.kolb-bot/workspace/` — 工作区（SOUL.md、记忆等）
