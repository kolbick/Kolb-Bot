...
Gateway 网关；如果无法访问，它会通过外部 `kolb-bot` CLI（无嵌入式运行时）启用 launchd
...
- 应用安装标记为 `bot.molt.gateway` 的按用户 LaunchAgent
  （使用 `--profile`/`KOLB_BOT_PROFILE` 时为 `bot.molt.<profile>`；支持旧版 `com.kolb-bot.*`）。
...
- 写入 `~/.kolb-bot/disable-launchagent`。
...
`--attach-only`（或 `--no-launchd`）启动它。这会设置 `~/.kolb-bot/disable-launchagent`，
