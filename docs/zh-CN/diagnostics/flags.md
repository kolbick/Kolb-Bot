...
```bash
ls -t /tmp/kolb-bot/kolb-bot-*.log | head -n 1
```

过滤 Telegram HTTP 诊断：

```bash
rg "telegram http error" /tmp/kolb-bot/kolb-bot-*.log
```

或在复现时使用 tail：

```bash
tail -f /tmp/kolb-bot/kolb-bot-$(date +%F).log | rg "telegram http error"
```

对于远程 Gateway 网关，你也可以使用 `kolb-bot logs --follow`（参见 [/cli/logs](/cli/logs)）。
