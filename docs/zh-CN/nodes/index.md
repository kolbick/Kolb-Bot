...
macOS 也可以在**节点模式**下运行：菜单栏应用连接到 Gateway 网关的 WS 服务器，并将其本地 canvas/camera 命令作为节点暴露（因此 `kolb-bot nodes …` 可以针对这台 Mac 工作）。
...
- `node.pair.*`（CLI：`kolb-bot nodes pending/approve/reject`）是一个单独的 Gateway 网关拥有的
...
- **批准**：通过 `~/.kolb-bot/exec-approvals.json` 在节点主机上执行。
...
- 令牌是 Gateway 网关配置中的 `gateway.auth.token`（Gateway 网关主机上的 `~/.kolb-bot/kolb-bot.json`）。
- `kolb-bot node run` 读取 `KOLB_BOT_GATEWAY_TOKEN` 进行认证。
...
- 在 `kolb-bot node run` / `kolb-bot node install` 上使用 `--display-name`（持久化在节点上的 `~/.kolb-bot/node.json` 中）。
- `kolb-bot nodes rename --node <id|name|ip> --name "Build Node"`（Gateway 网关覆盖）。
...
批准存储在节点主机的 `~/.kolb-bot/exec-approvals.json` 中。
...
kolb-bot nodes camera snap --node <idOrNameOrIp>            # 默认：两个朝向（2 个 MEDIA 行）
...
```bash
kolb-bot nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from KolbBot"}'
```
...
- 在无头节点主机上，`system.run` 受 exec 批准限制（`~/.kolb-bot/exec-approvals.json`）。
...
KolbBot 可以运行**无头节点主机**（无 UI），它连接到 Gateway 网关
...
- 节点主机将其节点 id、令牌、显示名称和 Gateway 网关连接信息存储在 `~/.kolb-bot/node.json` 中。
- Exec 批准通过 `~/.kolb-bot/exec-approvals.json` 在本地执行
  （参见 [Exec 批准](/tools/exec-approvals)）。
- 在 macOS 上，当配套应用 exec 主机可达时，无头节点主机优先使用它，
  如果应用不可用则回退到本地执行。设置 `KOLB_BOT_NODE_EXEC_HOST=app` 要求
  使用应用，或设置 `KOLB_BOT_NODE_EXEC_FALLBACK=0` 禁用回退。
- 当 Gateway 网关 WS 使用 TLS 时，添加 `--tls` / `--tls-fingerprint`。

## Mac 节点模式

- macOS 菜单栏应用作为节点连接到 Gateway 网关 WS 服务器（因此 `kolb-bot nodes …` 可以针对这台 Mac 工作）。
