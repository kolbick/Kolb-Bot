...
通过一键模板在 Railway 上部署 KolbBot，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/kolb-bot` — 控制面板 UI

## 你将获得

- 托管的 KolbBot Gateway网关 + 控制面板 UI
- `/setup` 网页设置向导（无需终端命令）
...
- `KOLB_BOT_STATE_DIR=/data/.kolb-bot`（推荐）
- `KOLB_BOT_WORKSPACE_DIR=/data/workspace`（推荐）
- `KOLB_BOT_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 KolbBot 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
