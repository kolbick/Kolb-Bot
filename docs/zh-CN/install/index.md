...
read_when:
  - 安装 KolbBot
  - 你想从 GitHub 安装
summary: 安装 KolbBot（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 kolb-bot、node-llama-cpp、sharp 等
pnpm add -g kolb-bot@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm kolb-bot ...` 运行仓库命令。
...
- 运行新手引导：`kolb-bot onboard --install-daemon`
- 快速检查：`kolb-bot doctor`
- 检查 Gateway 网关健康状态：`kolb-bot status` + `kolb-bot health`
- 打开仪表板：`kolb-bot dashboard`

## 安装方式：npm vs git（安装器）

安装器支持两种方式：

- `npm`（默认）：`npm install -g kolb-bot@latest`
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `kolb-bot`）。
