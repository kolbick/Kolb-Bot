...
- 如果是升级现有安装：运行 `kolb-bot doctor --non-interactive`（尽力执行）。
- 对于 git 安装：安装/更新后运行 `kolb-bot doctor --non-interactive`（尽力执行）。
...
如果你在**已有的 KolbBot 源码检出目录中**运行安装器（通过 `package.json` + `pnpm-workspace.yaml` 检测），它会提示：

- 更新并使用此检出（`git`）
- 或迁移到全局 npm 安装（`npm`）

在非交互式上下文中（无 TTY / `--no-prompt`），你必须传入 `--install-method git|npm`（或设置 `KOLB_BOT_INSTALL_METHOD`），否则脚本将以退出码 `2` 退出。
...
此脚本将 `kolb-bot` 安装到前缀目录（默认：`~/.kolb-bot`），同时在该前缀下安装专用的 Node 运行时，因此可以在不想改动系统 Node/npm 的机器上使用。
...
- 选择安装方式：
  - `npm`（默认）：`npm install -g kolb-bot@latest`
  - `git`：克隆/构建源码检出并安装包装脚本
- 在升级和 git 安装时运行 `kolb-bot doctor --non-interactive`（尽力执行）。
