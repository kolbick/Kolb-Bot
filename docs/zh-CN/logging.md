...
你可以在 `~/.kolb-bot/kolb-bot.json` 中覆盖此设置：
...
所有日志配置都在 `~/.kolb-bot/kolb-bot.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/kolb-bot/kolb-bot-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- KolbBot 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `kolb-bot plugins enable diagnostics-otel` 启用插件。
...
- `kolb-bot.tokens`（计数器，属性：`kolb-bot.token`、`kolb-bot.channel`、`kolb-bot.provider`、`kolb-bot.model`）
- `kolb-bot.cost.usd`（计数器，属性：`kolb-bot.channel`、`kolb-bot.provider`、`kolb-bot.model`）
- `kolb-bot.run.duration_ms`（直方图，属性：`kolb-bot.channel`、`kolb-bot.provider`、`kolb-bot.model`）
- `kolb-bot.context.tokens`（直方图，属性：`kolb-bot.context`、`kolb-bot.channel`、`kolb-bot.provider`、`kolb-bot.model`）

消息流：

- `kolb-bot.webhook.received`（计数器，属性：`kolb-bot.channel`、`kolb-bot.webhook`）
- `kolb-bot.webhook.error`（计数器，属性：`kolb-bot.channel`、`kolb-bot.webhook`）
- `kolb-bot.webhook.duration_ms`（直方图，属性：`kolb-bot.channel`、`kolb-bot.webhook`）
- `kolb-bot.message.queued`（计数器，属性：`kolb-bot.channel`、`kolb-bot.source`）
- `kolb-bot.message.processed`（计数器，属性：`kolb-bot.channel`、`kolb-bot.outcome`）
- `kolb-bot.message.duration_ms`（直方图，属性：`kolb-bot.channel`、`kolb-bot.outcome`）

队列 + 会话：

- `kolb-bot.queue.lane.enqueue`（计数器，属性：`kolb-bot.lane`）
- `kolb-bot.queue.lane.dequeue`（计数器，属性：`kolb-bot.lane`）
- `kolb-bot.queue.depth`（直方图，属性：`kolb-bot.lane` 或 `kolb-bot.channel=heartbeat`）
- `kolb-bot.queue.wait_ms`（直方图，属性：`kolb-bot.lane`）
- `kolb-bot.session.state`（计数器，属性：`kolb-bot.state`、`kolb-bot.reason`）
- `kolb-bot.session.stuck`（计数器，属性：`kolb-bot.state`）
- `kolb-bot.session.stuck_age_ms`（直方图，属性：`kolb-bot.state`）
- `kolb-bot.run.attempt`（计数器，属性：`kolb-bot.attempt`）
...
- `kolb-bot.webhook.error`
  - `kolb-bot.channel`、`kolb-bot.webhook`、`kolb-bot.chatId`、`kolb-bot.error`
- `kolb-bot.message.processed`
  - `kolb-bot.channel`、`kolb-bot.outcome`、`kolb-bot.chatId`、`kolb-bot.messageId`、`kolb-bot.sessionKey`、`kolb-bot.sessionId`、`kolb-bot.reason`
- `kolb-bot.session.stuck`
  - `kolb-bot.state`、`kolb-bot.ageMs`、`kolb-bot.queueDepth`、`kolb-bot.sessionKey`、`kolb-bot.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `kolb-bot doctor`。
