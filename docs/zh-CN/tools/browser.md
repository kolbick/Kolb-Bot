...
read_when:
  - 添加智能体控制的浏览器自动化
  - 调试 kolb-bot 干扰你自己 Chrome 的问题
  - 在 macOS 应用中实现浏览器设置和生命周期管理
summary: 集成浏览器控制服务 + 操作命令
title: 浏览器（KolbBot 托管）
x-i18n:
...
KolbBot 可以运行一个由智能体控制的**专用 Chrome/Brave/Edge/Chromium 配置文件**。
它与你的个人浏览器隔离，通过 Gateway 网关内部的小型本地控制服务进行管理（仅限 loopback）。

新手视角：

- 把它想象成一个**独立的、仅供智能体使用的浏览器**。
- `kolb-bot` 配置文件**不会**触及你的个人浏览器配置文件。
- 智能体可以在安全的通道中**打开标签页、读取页面、点击和输入**。
- 默认的 `chrome` 配置文件通过扩展中继使用**系统默认的 Chromium 浏览器**；切换到 `kolb-bot` 可使用隔离的托管浏览器。

## 功能概览

- 一个名为 **kolb-bot** 的独立浏览器配置文件（默认橙色主题）。
- 确定性标签页控制（列出/打开/聚焦/关闭）。
- 智能体操作（点击/输入/拖动/选择）、快照、截图、PDF。
- 可选的多配置文件支持（`kolb-bot`、`work`、`remote` 等）。
...
## 配置文件：`kolb-bot` 与 `chrome`

- `kolb-bot`：托管的隔离浏览器（无需扩展）。
- `chrome`：到你**系统浏览器**的扩展中继（需要将 KolbBot 扩展附加到标签页）。

如果你希望默认使用托管模式，请设置 `browser.defaultProfile: "kolb-bot"`。

## 配置

浏览器设置位于 `~/.kolb-bot/kolb-bot.json`。
...
- 如果你覆盖了 Gateway 网关端口（`gateway.port` 或 `KOLB_BOT_GATEWAY_PORT`），派生的浏览器端口会相应调整以保持在同一"系列"中。
- 未设置时，`cdpUrl` 默认为中继端口。
...
- 默认配置文件是 `chrome`（扩展中继）。使用 `defaultProfile: "kolb-bot"` 来使用托管浏览器。
- 自动检测顺序：如果系统默认浏览器是基于 Chromium 的则使用它；否则 Chrome → Brave → Edge → Chromium → Chrome Canary。
- 本地 `kolb-bot` 配置文件会自动分配 `cdpPort`/`cdpUrl` — 仅为远程 CDP 设置这些。

## 使用 Brave（或其他基于 Chromium 的浏览器）

如果你的**系统默认**浏览器是基于 Chromium 的（Chrome/Brave/Edge 等），KolbBot 会自动使用它。设置 `browser.executablePath` 可覆盖自动检测：
...
- **远程 CDP：** 设置 `browser.profiles.<name>.cdpUrl`（或 `browser.cdpUrl`）以附加到远程的基于 Chromium 的浏览器。在这种情况下，KolbBot 不会启动本地浏览器。
...
KolbBot 在调用 `/json/*` 端点和连接 CDP WebSocket 时会保留认证信息。建议使用环境变量或密钥管理器存储令牌，而不是将其提交到配置文件中。

## 节点浏览器代理（零配置默认）

如果你在有浏览器的机器上运行**节点主机**，KolbBot 可以自动将浏览器工具调用路由到该节点，无需任何额外的浏览器配置。这是远程 Gateway 网关的默认路径。
...
[Browserless](https://browserless.io) 是一个托管的 Chromium 服务，通过 HTTPS 暴露 CDP 端点。你可以将 KolbBot 浏览器配置文件指向 Browserless 区域端点，并使用你的 API 密钥进行认证。
...
KolbBot 支持多个命名配置文件（路由配置）。配置文件可以是：

- **kolb-bot 托管**：具有独立用户数据目录和 CDP 端口的专用基于 Chromium 的浏览器实例
- **远程**：显式 CDP URL（在其他地方运行的基于 Chromium 的浏览器）
- **扩展中继**：通过本地中继 + Chrome 扩展访问你现有的 Chrome 标签页

默认值：

- 如果缺少 `kolb-bot` 配置文件，会自动创建。
...
KolbBot 还可以通过本地 CDP 中继 + Chrome 扩展驱动**你现有的 Chrome 标签页**（无需单独的"kolb-bot"Chrome 实例）。
...
- 你点击标签页上的 **KolbBot Browser Relay** 扩展图标来附加（它不会自动附加）。
...
- "加载已解压的扩展程序" → 选择 `kolb-bot browser extension path` 打印的目录
...
某些功能（navigate/act/AI 快照/角色快照、元素截图、PDF）需要 Playwright。如果未安装 Playwright，这些端点会返回明确的 501 错误。ARIA 快照和基本截图对于 kolb-bot 托管的 Chrome 仍然有效。对于 Chrome 扩展中继驱动程序，ARIA 快照和截图需要 Playwright。

如果你看到 `Playwright is not available in this gateway build`，请安装完整的 Playwright 包（不是 `playwright-core`）并重启 Gateway 网关，或者重新安装带浏览器支持的 KolbBot。
...
要持久化浏览器下载，设置 `PLAYWRIGHT_BROWSERS_PATH`（例如 `/home/node/.cache/ms-playwright`）并确保 `/home/node` 通过 `KOLB_BOT_HOME_VOLUME` 或绑定挂载持久化。参见 [Docker](/install/docker)。
...
- `kolb-bot browser download e12 /tmp/report.pdf`
- `kolb-bot browser waitfordownload /tmp/report.pdf`
...
- `kolb-bot browser set headers --json '{"X-Debug":"1"}'`
...
- `snapshot`：
  ...
  - 配置默认值（仅限工具/CLI）：设置 `browser.snapshotDefaults.mode: "efficient"` 以在调用者未传递模式时使用高效快照（参见 [Gateway 网关配置](/gateway/configuration#browser-kolb-bot-managed-browser)）。
...
- **AI 快照（数字 ref）**：`kolb-bot browser snapshot`（默认；`--format ai`）
  - 输出：包含数字 ref 的文本快照。
  - 操作：`kolb-bot browser click 12`、`kolb-bot browser type 23 "hello"`。
  - 内部通过 Playwright 的 `aria-ref` 解析 ref。

- **角色快照（角色 ref 如 `e12`）**：`kolb-bot browser snapshot --interactive`（或 `--compact`、`--depth`、`--selector`、`--frame`）
  - 输出：带有 `[ref=e12]`（和可选的 `[nth=1]`）的基于角色的列表/树。
  - 操作：`kolb-bot browser click e12`、`kolb-bot browser highlight e12`。
...
3. 如果仍然失败：`kolb-bot browser highlight <ref>` 查看 Playwright 定位的目标
4. 如果页面行为异常：
   - `kolb-bot browser errors --clear`
   - `kolb-bot browser requests --filter api --clear`
5. 深度调试：录制 trace：
   - `kolb-bot browser trace start`
   - 重现问题
   - `kolb-bot browser trace stop`（打印 `TRACE:<path>`）
...
- kolb-bot 浏览器配置文件可能包含已登录的会话；请将其视为敏感信息。
- `browser act kind=evaluate` / `kolb-bot browser evaluate` 和 `wait --fn` 在页面上下文中执行任意 JavaScript。提示注入可能会操纵它。如果不需要，请使用 `browser.evaluateEnabled=false` 禁用它。
...
- `browser` 接受：
  - `profile` 来选择命名的浏览器配置文件（kolb-bot、chrome 或远程 CDP）。
