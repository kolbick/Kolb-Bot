...
- [快速开始与首次运行设置](#quick-start-and-firstrun-setup)
  ...
  - [无法访问 docs.kolb-bot.ai（SSL 错误），怎么办？](#i-cant-access-docskolb-botai-ssl-error-what-now)
...
### 无法访问 docs.kolb-bot.ai（SSL 错误），怎么办

一些 Comcast/Xfinity 连接通过 Xfinity Advanced Security 错误地拦截了 `docs.kolb-bot.ai`。禁用该功能或将 `docs.kolb-bot.ai` 加入白名单，然后重试。更多详情：[故障排除](/help/troubleshooting#docskolb-botai-shows-an-ssl-error-comcastxfinity)。
...
```bash
curl -fsSL --proto '=https' --tlsv1.2 https://kolb-bot.ai/install.sh | bash -s -- --beta
```

```bash
curl -fsSL --proto '=https' --tlsv1.2 https://kolb-bot.ai/install.sh | bash -s -- --install-method git
```

Windows 安装程序（PowerShell）：
https://kolb-bot.ai/install.ps1
...
```bash
curl -fsSL https://kolb-bot.ai/install.sh | bash -s -- --verbose
```

带详细输出的 Beta 安装：

```bash
curl -fsSL https://kolb-bot.ai/install.sh | bash -s -- --beta --verbose
```

可编辑（git）安装：

```bash
curl -fsSL https://kolb-bot.ai/install.sh | bash -s -- --install-method git --verbose
```
