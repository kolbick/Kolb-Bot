...
function createProfileContext(
  ...
  const ensureBrowserAvailable = async (): Promise<void> => {
    ...
    if (isExtension) {
      ...
      throw new Error(
        `Chrome extension relay is running, but no tab is connected. Click the KolbBot Chrome extension icon on a tab to attach it (profile "${profile.name}").`,
      );
    }
      ...
      if (profile.driver === "extension") {
        throw new Error(
          `tab not found (no attached Chrome tabs for profile "${profile.name}"). ` +
            "Click the KolbBot Browser Relay toolbar icon on the tab you want to control (badge ON).",
        );
      }
