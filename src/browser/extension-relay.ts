...
async function looksLikeKolbBotRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/KolbBot-Extension-Relay",
          revision: "0",
          userAgent: "KolbBot-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "KolbBot/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeKolbBotRelay(info.baseUrl))) {
