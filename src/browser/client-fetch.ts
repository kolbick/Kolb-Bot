...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  ...
  const operatorHint = isLocal
    ? `Restart the KolbBot gateway (KolbBot.app menubar, or \`${formatCliCommand("kolb-bot gateway")}\`).`
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the KolbBot browser control service (timed out after ${timeoutMs}ms). ${operatorHint} ${modelHint}`,
    );
  }
  return new Error(
    `Can't reach the KolbBot browser control service. ${operatorHint} ${modelHint} (${msg})`,
  );
}
