...
vi.mock("./chrome.js", () => ({
  ...
  resolveKolbBotUserDataDir: vi.fn(() => "/tmp/kolb-bot"),
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (kolb-bot)
    expect(status.profile).toBe("kolb-bot");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("kolb-bot");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default kolb-bot profile
    expect(result.profiles.some((p) => p.name === "kolb-bot")).toBe(true);
  });

  it("GET /tabs?profile=kolb-bot returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=kolb-bot`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=kolb-bot opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=kolb-bot`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
