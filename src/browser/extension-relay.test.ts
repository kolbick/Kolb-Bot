...
describe("chrome extension relay server", () => {
  ...
  beforeEach(() => {
    previousGatewayToken = process.env.KOLB_BOT_GATEWAY_TOKEN;
  ...
  afterEach(async () => {
    ...
    if (previousGatewayToken === undefined) {
      ...
      process.env.KOLB_BOT_GATEWAY_TOKEN = previousGatewayToken;
    }
  });
  ...
  it("uses gateway token for relay auth headers on loopback URLs", async () => {
    ...
    expect(headers["x-kolb-bot-relay-token"]).toBe(TEST_GATEWAY_TOKEN);
  });
