...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-update-"));
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "kolb-bot", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "kolb-bot", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "kolb-bot.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g kolb-bot@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
    ...
    expect(calls.some((call) => call === "npm i -g kolb-bot@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g kolb-bot@beta")).toBe(true);
  });
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g kolb-bot@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "kolb-bot", version: "2.0.0" }),
            "utf-8",
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "kolb-bot.mjs")} doctor --non-interactive`
      ) {
