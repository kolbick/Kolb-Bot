...
describe("runGatewayUpdate", () => {
  ...
  function createStableTagRunner(params: {
    ...
    const calls: string[] = [];
    let uiBuildCount = 0;
    const doctorKey = `${process.execPath} ${path.join(tempDir, "kolb-bot.mjs")} doctor --non-interactive --fix`;
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "kolb-bot.mjs")} doctor --non-interactive --fix`]:
        {
          stdout: "",
        },
    });
  ...
  it("cleans stale npm rename dirs before global update", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g kolb-bot@latest --no-fund --no-audit --loglevel=error") {
