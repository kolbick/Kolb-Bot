...
describe("control UI assets helpers", () => {
  ...
  it("resolves control-ui root for dist/gateway bundle argv1", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-ui-"));
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "kolb-bot" }));
  ...
  it("resolves dist control-ui index path from package root argv1", async () => {
    ...
    try {
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "kolb-bot" }));
      await fs.writeFile(path.join(tmp, "kolb-bot.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(tmp, "kolb-bot.mjs"))).toBe(
  ...
  it("resolves control-ui root for package entrypoint argv1", async () => {
    ...
    try {
      ...
      expect(resolveControlUiRootSync({ argv1: path.join(tmp, "kolb-bot.mjs") })).toBe(
        path.join(tmp, "dist", "control-ui"),
      );
    } finally {
      await fs.rm(tmp, { recursive: true, force: true });
  ...
  it("resolves dist control-ui index path from .bin argv1", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(tmp, "node_modules", "kolb-bot");
      await fs.mkdir(binDir, { recursive: true });
      await fs.mkdir(path.join(pkgRoot, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(binDir, "kolb-bot"), "#!/usr/bin/env node\n");
      await fs.writeFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "kolb-bot" }));
      await fs.writeFile(path.join(pkgRoot, "dist", "control-ui", "index.html"), "<html></html>\n");

      expect(await resolveControlUiDistIndexPath(path.join(binDir, "kolb-bot"))).toBe(
  ...
  it("resolves via fallback when package root resolution fails but package name matches", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-ui-"));
    try {
      // Package named "kolb-bot" but resolveKolbBotPackageRoot failed for other reasons
      await fs.writeFile(path.join(tmp, "package.json"), JSON.stringify({ name: "kolb-bot" }));
      await fs.writeFile(path.join(tmp, "kolb-bot.mjs"), "export {};\n");
      await fs.mkdir(path.join(tmp, "dist", "control-ui"), { recursive: true });
      await fs.writeFile(path.join(tmp, "dist", "control-ui", "index.html"), "<html></html>\n");
  ...
  it("returns null when package name does not match kolb-bot", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-ui-"));
    try {
