...
import { ensureKolbBotCliOnPath } from "./path-env.js";

describe("ensureKolbBotCliOnPath", () => {
  it("prepends the bundled app bin dir when a sibling kolb-bot exists", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-path-"));
    try {
      ...
      const originalFlag = process.env.KOLB_BOT_PATH_BOOTSTRAPPED;
      ...
      try {
        ...
        if (originalFlag === undefined) {
          ...
          process.env.KOLB_BOT_PATH_BOOTSTRAPPED = originalFlag;
        }
      }
    } finally {
