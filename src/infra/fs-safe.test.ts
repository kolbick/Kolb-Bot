...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await makeTempDir("kolb-bot-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await makeTempDir("kolb-bot-fs-safe-root-");
    const outside = await makeTempDir("kolb-bot-fs-safe-outside-");
