...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "KolbBot";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(KolbBot\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.KOLB_BOT_MDNS_HOSTNAME?.trim() || "kolb-bot";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
