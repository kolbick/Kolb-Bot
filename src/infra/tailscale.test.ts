...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.KOLB_BOT_TEST_TAILSCALE_BINARY;
  ...
  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.KOLB_BOT_TEST_TAILSCALE_BINARY;
    } else {
      process.env.KOLB_BOT_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
