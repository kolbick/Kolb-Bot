...
function warnIfConfigFromFuture(cfg: KolbBotConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (cmp < 0) {
    logger.warn(
      `Config was last written by a newer KolbBot (${touched}); current version is ${VERSION}.`,
    );
  }
}

function applyConfigEnv(cfg: KolbBotConfig, env: NodeJS.ProcessEnv): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): KolbBotConfig {
    try {
      ...
      if (resolved && typeof resolved === "object" && "env" in resolved) {
        applyConfigEnv(resolved as KolbBotConfig, deps.env);
      }
  ...
  async function writeConfigFile(cfg: KolbBotConfig) {
    clearConfigCache();
...
export async function writeConfigFile(cfg: KolbBotConfig): Promise<void> {
  clearConfigCache();
