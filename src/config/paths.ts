...
const LEGACY_STATE_DIRNAMES = [".kolb-bot", ".moltbot", ".moldbot"] as const;
const NEW_STATE_DIRNAME = ".kolb-bot";
const CONFIG_FILENAME = "kolb-bot.json";
const LEGACY_CONFIG_FILENAMES = ["kolb-bot.json", "moltbot.json", "moldbot.json"] as const;
...
export function resolveStateDir(
  ...
  const override = env.KOLB_BOT_STATE_DIR?.trim() || env.CLAWDBOT_STATE_DIR?.trim();
  if (override) {
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.KOLB_BOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
...
export function resolveDefaultConfigCandidates(
  ...
  const explicit = env.KOLB_BOT_CONFIG_PATH?.trim() || env.CLAWDBOT_CONFIG_PATH?.trim();
  if (explicit) {
