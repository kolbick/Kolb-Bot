...
describe("sessions", () => {
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.KOLB_BOT_STATE_DIR = "/custom/state";
    try {
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const prev = process.env.KOLB_BOT_STATE_DIR;
    const stateDir = path.resolve("/home/user/.kolb-bot");
  ...
  it("resolves cross-agent paths when KOLB_BOT_STATE_DIR differs from stored paths", () => {
    const prev = process.env.KOLB_BOT_STATE_DIR;
    process.env.KOLB_BOT_STATE_DIR = path.resolve("/different/state");
    try {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    const prev = process.env.KOLB_BOT_STATE_DIR;
    process.env.KOLB_BOT_STATE_DIR = path.resolve("/home/user/.kolb-bot");
    try {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
