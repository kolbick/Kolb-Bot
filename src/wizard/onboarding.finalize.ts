...
export async function finalizeOnboardingWizard(
  ...
  if (!opts.skipUi && gatewayProbe.ok) {
    ...
    await prompter.note(
      [
        ...
        "Stored in: ~/.kolb-bot/kolb-bot.json (gateway.auth.token) or KOLB_BOT_GATEWAY_TOKEN.",
        ...
        "Web UI stores a copy in this browser's localStorage (kolb-bot.control.settings.v1).",
    ...
    if (hatchChoice === "tui") {
      ...
      await prompter.note(
        [
          ...
          controlUiOpened
            ? "Opened in your browser. Keep that tab to control KolbBot."
            : "Copy/paste this URL in a browser on this machine to control KolbBot.",
          controlUiOpenHint,
        ]
          .filter(Boolean)
          .join("\n"),
  ...
  await prompter.note(
    hasWebSearchKey
      ...
      : [
          ...
          "",
          "KolbBot uses Brave Search for the `web_search` tool. Without a Brave Search API key, web search won’t work.",
          "",
          "Set it up interactively:",
          `- Run: ${formatCliCommand("kolb-bot configure --section web")}`,
  ...
  await prompter.outro(
    controlUiOpened
      ? "Onboarding complete. Dashboard opened; keep that tab to control KolbBot."
      : seededInBackground
        ? "Onboarding complete. Web UI seeded in the background; open it anytime with the dashboard link above."
        : "Onboarding complete. Use the dashboard link above to control KolbBot.",
  );
