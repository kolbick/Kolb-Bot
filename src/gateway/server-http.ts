...
export function createHooksRequestHandler(
  ...
  return async (req, res) => {
    ...
    if (url.searchParams.has("token")) {
      ...
      res.end(
        "Hook token must be provided via Authorization: Bearer <token> or X-KolbBot-Token header (query parameters are not allowed).",
      );
      return true;
    }
...
export function attachGatewayUpgradeHandler(opts: {
  ...
  const { httpServer, wss, relayWss, canvasHost, clients, resolvedAuth, rateLimiter } = opts;
  httpServer.on("upgrade", (req, socket, head) => {
    void (async () => {
      ...
      if (relayWss && handleRelayUpgrade(relayWss, req, socket, head)) {
        return;
      }
