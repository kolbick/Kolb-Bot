...
beforeAll(async () => {
  previousToken = process.env.KOLB_BOT_GATEWAY_TOKEN;
  process.env.KOLB_BOT_GATEWAY_TOKEN = "test-gateway-token-1234567890";
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.KOLB_BOT_GATEWAY_TOKEN = previousToken;
  }
});
...
test.each([
  { clientId: "kolb-bot-ios", platform: "ios" },
  { clientId: "kolb-bot-android", platform: "android" },
