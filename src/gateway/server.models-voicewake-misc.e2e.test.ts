...
describe("gateway server models + voicewake", () => {
  const setTempHome = (homeDir: string) => {
    ...
    process.env.KOLB_BOT_STATE_DIR = path.join(homeDir, ".kolb-bot");
  ...
  test(
    ...
    { timeout: 60_000 },
    async () => {
      const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-home-"));
...
describe("gateway server misc", () => {
  test("hello-ok advertises the gateway port for canvas host", async () => {
    ...
    const prevCanvasPort = process.env.KOLB_BOT_CANVAS_HOST_PORT;
    process.env.KOLB_BOT_GATEWAY_TOKEN = "secret";
    testTailnetIPv4.value = "100.64.0.1";
    testState.gatewayBind = "lan";
    const canvasPort = await getFreePort();
    testState.canvasHostPort = canvasPort;
    process.env.KOLB_BOT_CANVAS_HOST_PORT = String(canvasPort);
    ...
    if (prevCanvasPort === undefined) {
      ...
      process.env.KOLB_BOT_CANVAS_HOST_PORT = prevCanvasPort;
    }
  });
