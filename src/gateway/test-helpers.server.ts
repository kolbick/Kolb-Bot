...
async function setupGatewayTestHome() {
  ...
  previousConfigPath = process.env.KOLB_BOT_CONFIG_PATH;
  previousSkipBrowserControl = process.env.KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.KOLB_BOT_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.KOLB_BOT_SKIP_CANVAS_HOST;
  previousBundledPluginsDir = process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
  previousSkipChannels = process.env.KOLB_BOT_SKIP_CHANNELS;
  previousSkipProviders = process.env.KOLB_BOT_SKIP_PROVIDERS;
  previousSkipCron = process.env.KOLB_BOT_SKIP_CRON;
  previousMinimalGateway = process.env.KOLB_BOT_TEST_MINIMAL_GATEWAY;
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "kolb-bot-test-"));
  } else {
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousConfigPath === undefined) {
      ...
      process.env.KOLB_BOT_CONFIG_PATH = previousConfigPath;
    }
    if (previousSkipBrowserControl === undefined) {
      delete process.env.KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER;
    } else {
      process.env.KOLB_BOT_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    }
    if (previousSkipGmailWatcher === undefined) {
      delete process.env.KOLB_BOT_SKIP_GMAIL_WATCHER;
    } else {
      process.env.KOLB_BOT_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    }
    if (previousSkipCanvasHost === undefined) {
      delete process.env.KOLB_BOT_SKIP_CANVAS_HOST;
    } else {
      process.env.KOLB_BOT_SKIP_CANVAS_HOST = previousSkipCanvasHost;
    }
    if (previousBundledPluginsDir === undefined) {
      delete process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
    } else {
      process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
    }
    if (previousSkipChannels === undefined) {
      delete process.env.KOLB_BOT_SKIP_CHANNELS;
    } else {
      process.env.KOLB_BOT_SKIP_CHANNELS = previousSkipChannels;
    }
    if (previousSkipProviders === undefined) {
      delete process.env.KOLB_BOT_SKIP_PROVIDERS;
    } else {
      process.env.KOLB_BOT_SKIP_PROVIDERS = previousSkipProviders;
    }
    if (previousSkipCron === undefined) {
      delete process.env.KOLB_BOT_SKIP_CRON;
    } else {
      process.env.KOLB_BOT_SKIP_CRON = previousSkipCron;
    }
    if (previousMinimalGateway === undefined) {
      delete process.env.KOLB_BOT_TEST_MINIMAL_GATEWAY;
    } else {
      process.env.KOLB_BOT_TEST_MINIMAL_GATEWAY = previousMinimalGateway;
    }
  }
