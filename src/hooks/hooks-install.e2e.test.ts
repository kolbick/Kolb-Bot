...
describe("hooks install (e2e)", () => {
  ...
  beforeEach(async () => {
    ...
    prevStateDir = process.env.KOLB_BOT_STATE_DIR;
    prevBundledDir = process.env.KOLB_BOT_BUNDLED_HOOKS_DIR;
    process.env.KOLB_BOT_STATE_DIR = path.join(baseDir, "state");
    process.env.KOLB_BOT_BUNDLED_HOOKS_DIR = path.join(baseDir, "bundled-none");
    vi.resetModules();
  });
  ...
  afterEach(async () => {
    ...
    if (prevBundledDir === undefined) {
      delete process.env.KOLB_BOT_BUNDLED_HOOKS_DIR;
    } else {
      process.env.KOLB_BOT_BUNDLED_HOOKS_DIR = prevBundledDir;
    }
