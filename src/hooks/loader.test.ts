...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `kolb-bot-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.KOLB_BOT_BUNDLED_HOOKS_DIR;
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      ...
      process.env.KOLB_BOT_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
