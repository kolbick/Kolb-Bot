...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-win-"));
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "kolb-bot.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-discord-"));
    process.env.KOLB_BOT_STATE_DIR = tmp;
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "kolb-bot-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.KOLB_BOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.KOLB_BOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-discord-open-"));
    process.env.KOLB_BOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: KolbBotConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.KOLB_BOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-slack-"));
    process.env.KOLB_BOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: KolbBotConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.KOLB_BOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-slack-open-"));
    process.env.KOLB_BOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: KolbBotConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.KOLB_BOT_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-telegram-"));
    process.env.KOLB_BOT_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: KolbBotConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      configPath: "/Users/test/Dropbox/.kolb-bot/kolb-bot.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-security-audit-"));
    const stateDir = path.join(tmp, "state");
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-audit-scanner-"));
...
`,
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.KOLB_BOT_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.KOLB_BOT_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.KOLB_BOT_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.KOLB_BOT_GATEWAY_PASSWORD;
      } else {
        process.env.KOLB_BOT_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env password over remote password", async () => {
      process.env.KOLB_BOT_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: KolbBotConfig = {
        gateway: {
          mode: "remote",
