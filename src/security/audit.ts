  ...
  if (stateDirPerms.ok) {
    ...
    if (stateDirPerms.worldWritable) {
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; other users can write into your KolbBot state.`,
      ...
      findings.push({
        ...
        detail: `${formatPermissionDetail(params.stateDir, stateDirPerms)}; group users can write into your KolbBot state.`,
        remediation: formatPermissionRemediation({
          targetPath: params.stateDir,
          perms: stateDirPerms,
          isDir: true,
...
function collectBrowserControlFindings(cfg: KolbBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
    ...
    const token =
      mode === "remote"
        ? typeof remote?.token === "string" && remote.token.trim()
          ? remote.token.trim()
          : undefined
        : process.env.KOLB_BOT_GATEWAY_TOKEN?.trim() ||
          (typeof authToken === "string" && authToken.trim() ? authToken.trim() : undefined);
    const password =
      process.env.KOLB_BOT_GATEWAY_PASSWORD?.trim() ||
      (mode === "remote"
