...
export function collectHooksHardeningFindings(cfg: KolbBotConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function listWorkspaceDirs(cfg: KolbBotConfig): string[] {
  ...
  for (const pluginName of pluginDirs) {
    ...
    if (summary.critical > 0) {
      ...
      findings.push({
        ...
        remediation:
          "Review the plugin source code carefully before use. If untrusted, remove the plugin from your KolbBot extensions state directory.",
      });
