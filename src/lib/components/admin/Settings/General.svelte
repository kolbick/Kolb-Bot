		...
		{#if adminConfig !== null}
			<AdminSettingSection first>
				...
				<div class="text-xs">
					<div class="text-gray-600 dark:text-gray-400">{$i18n.t('License')}</div>
						...
						<span class="mt-0.5 block text-gray-400 dark:text-gray-600">
							{$i18n.t(
								'Private self-hosted deployment. See the LICENSE file and docs/LICENSE_NOTES.md in the repository for license details.'
							)}
						</span>
				</div>
			</AdminSettingSection>
			...
			<AdminSettingSection title={$i18n.t('Features')}>
				<AdminSettingRow
					...
					description={$i18n.t('Allow users to share chats with the Kolb-Bot community.')}
				>
