...
function restoreKolbBotStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.KOLB_BOT_STATE_DIR;
    return;
  }
  process.env.KOLB_BOT_STATE_DIR = originalValue;
}

const originalStateDir = process.env.KOLB_BOT_STATE_DIR;

afterEach(() => {
  restoreKolbBotStateDir(originalStateDir);
});
