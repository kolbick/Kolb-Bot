...
const originalKolbBotStateDir = process.env.KOLB_BOT_STATE_DIR;

afterEach(() => {
  if (originalKolbBotStateDir === undefined) {
    delete process.env.KOLB_BOT_STATE_DIR;
  } else {
    process.env.KOLB_BOT_STATE_DIR = originalKolbBotStateDir;
  }
});
