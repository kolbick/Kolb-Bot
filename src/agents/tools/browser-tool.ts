...
function resolveBrowserBaseUrl(params: {
  ...
  if (!resolved.enabled) {
    throw new Error(
      "Browser control is disabled. Set browser.enabled=true in ~/.kolb-bot/kolb-bot.json.",
    );
  }
  return undefined;
}
...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    name: "browser",
    description: [
      "Control the browser via KolbBot's browser control server (status/start/stop/profiles/tabs/open/snapshot/screenshot/actions).",
      ...
      "Chrome extension relay needs an attached tab: user must click the KolbBot Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it.",
        ...
        case "act": {
          ...
          try {
            ...
            if (msg.includes("404:") && msg.includes("tab not found") && profile === "chrome") {
              ...
              if (!tabs.length) {
                throw new Error(
                  "No Chrome tabs are attached via the KolbBot Browser Relay extension. Click the toolbar icon on the tab you want to control (badge ON), then retry.",
                  { cause: err },
                );
              }
              throw new Error(
