...
type CreateKolbBotTools = (typeof import("./kolb-bot-tools.js"))["createKolbBotTools"];
type CreateKolbBotToolsOpts = Parameters<CreateKolbBotTools>[0];

async function getSessionsSpawnTool(opts: CreateKolbBotToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createKolbBotTools } = await import("./kolb-bot-tools.js");
  const tool = createKolbBotTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
