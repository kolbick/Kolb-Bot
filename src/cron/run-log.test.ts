...
describe("cron run log", () => {
  ...
  it("appends JSONL and prunes by line count", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-cron-log-"));
  ...
  it("reads newest entries and filters by jobId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-cron-log-read-"));
  ...
  it("reads telemetry fields", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-cron-log-telemetry-"));
    const logPath = path.join(dir, "runs", "job-1.jsonl");
