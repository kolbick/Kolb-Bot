...
describe("edge cases", () => {
  ...
  it("keeps .co domains as links", () => {
    const result = markdownToTelegramHtml("Visit t.co and kolb-bot.co");
    expect(result).toContain('<a href="http://t.co">');
    expect(result).toContain('<a href="http://kolb-bot.co">');
    expect(result).not.toContain("<code>t.co</code>");
    expect(result).not.toContain("<code>kolb-bot.co</code>");
  });
