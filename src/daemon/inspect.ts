...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "kolb-bot" | "kolb-bot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["kolb-bot", "kolb-bot", "moltbot"] as const;
...
function isLegacyLabel(label: string): boolean {
  ...
  return lower.includes("kolb-bot") || lower.includes("moltbot");
}
  ...
  for (const entry of entries) {
    ...
    if (!marker) {
      ...
      results.push({
        ...
        scope: params.scope,
        marker: isLegacyLabel(label) ? "kolb-bot" : "moltbot",
        legacy: true,
      });
      continue;
    }
