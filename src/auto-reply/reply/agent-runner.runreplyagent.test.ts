...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { KOLB_BOT_STATE_DIR: process.env.KOLB_BOT_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.KOLB_BOT_STATE_DIR === undefined) {
      delete process.env.KOLB_BOT_STATE_DIR;
    } else {
      process.env.KOLB_BOT_STATE_DIR = snapshot.KOLB_BOT_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "kolb-bot-typing-heartbeat-"));
  });
