...
beforeEach(async () => {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-web-home-"));
...
describe("web auto-reply", () => {
  ...
  it("applies responsePrefix to regular replies", async () => {
    ...
    expect(reply).toHaveBeenCalledWith("☠️ hello there");
    resetLoadConfigMock();
  });
        ...
        list: [
          {
            id: "main",
            default: true,
            identity: { name: "Mainbot", emoji: "☠️", theme: "pirate skull" },
          },
        ],
      },
  ...
  it("does not double-prefix if responsePrefix already present", async () => {
    ...
    const resolver = vi.fn().mockResolvedValue({ text: "☠️ already prefixed" });
    ...
    expect(reply).toHaveBeenCalledWith("☠️ already prefixed");
    resetLoadConfigMock();
  });
});
