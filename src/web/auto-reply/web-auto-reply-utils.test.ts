...
describe("resolveMentionTargets with @lid mapping", () => {
  ...
  beforeAll(async () => {
    authDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-lid-mapping-"));
...
describe("getSessionSnapshot", () => {
  it("uses channel reset overrides when configured", async () => {
    ...
    try {
      const root = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-snapshot-"));
