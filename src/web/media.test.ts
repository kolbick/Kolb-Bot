...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-media-test-"));
...
describe("local media root guard", () => {
  ...
  it("allows default KolbBot state workspace and sandbox roots", async () => {
  ...
  it("rejects default KolbBot state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
