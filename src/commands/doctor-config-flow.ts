...
function stripUnknownConfigKeys(config: KolbBotConfig): {
...
function noteOpencodeProviderOverrides(cfg: KolbBotConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("kolb-bot doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
