...
beforeAll(() => {
  previousProfile = process.env.KOLB_BOT_PROFILE;
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.KOLB_BOT_PROFILE = previousProfile;
  }
});
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("KolbBot status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("kolb-bot status --all") ||
          l.includes("kolb-bot --profile isolated status --all") ||
          l.includes("kolb-bot status --all") ||
          l.includes("kolb-bot --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.KOLB_BOT_GATEWAY_TOKEN = "abcd1234";
    try {
