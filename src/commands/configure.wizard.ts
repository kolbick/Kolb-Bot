...
async function runGatewayHealthCheck(params: {
  ...
  const token = params.cfg.gateway?.auth?.token ?? process.env.KOLB_BOT_GATEWAY_TOKEN;
  const password = params.cfg.gateway?.auth?.password ?? process.env.KOLB_BOT_GATEWAY_PASSWORD;
...
export async function runConfigureWizard(
  ...
  try {
    ...
    intro(opts.command === "update" ? "KolbBot update wizard" : "KolbBot configure");
    ...
    const localProbe = await probeGatewayReachable({
      url: localUrl,
      token: baseConfig.gateway?.auth?.token ?? process.env.KOLB_BOT_GATEWAY_TOKEN,
      password: baseConfig.gateway?.auth?.password ?? process.env.KOLB_BOT_GATEWAY_PASSWORD,
    });
    ...
    const newPassword = nextConfig.gateway?.auth?.password ?? process.env.KOLB_BOT_GATEWAY_PASSWORD;
    const oldPassword = baseConfig.gateway?.auth?.password ?? process.env.KOLB_BOT_GATEWAY_PASSWORD;
    const token = nextConfig.gateway?.auth?.token ?? process.env.KOLB_BOT_GATEWAY_TOKEN;
