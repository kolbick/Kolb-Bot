...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.KOLB_BOT_GATEWAY_PASSWORD,
    disableConfigCache: process.env.KOLB_BOT_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("KOLB_BOT_STATE_DIR", prev.stateDir);
  restoreEnvVar("KOLB_BOT_CONFIG_PATH", prev.configPath);
  restoreEnvVar("KOLB_BOT_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("KOLB_BOT_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("KOLB_BOT_SKIP_CRON", prev.skipCron);
  restoreEnvVar("KOLB_BOT_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("KOLB_BOT_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("KOLB_BOT_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("KOLB_BOT_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("kolb-bot-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("kolb-bot-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("kolb-bot-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("kolb-bot-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("kolb-bot-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
