...
function setSlackDmPolicy(cfg: KolbBotConfig, dmPolicy: DmPolicy) {
  const allowFrom =
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "KolbBot";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for KolbBot`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: KolbBotConfig, allowFrom: string[]): KolbBotConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
