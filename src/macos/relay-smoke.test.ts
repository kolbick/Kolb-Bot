...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { KOLB_BOT_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { KOLB_BOT_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports KOLB_BOT_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { KOLB_BOT_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { KOLB_BOT_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { KOLB_BOT_SMOKE: "nope" })).toBe("qr");
  });
});
