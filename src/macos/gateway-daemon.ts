...
const BUNDLED_VERSION =
  (typeof __KOLB_BOT_VERSION__ === "string" && __KOLB_BOT_VERSION__) ||
  process.env.KOLB_BOT_BUNDLED_VERSION ||
  "0.0.0";
...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `kolb-bot --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.KOLB_BOT_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.KOLB_BOT_GATEWAY_BIND ??
        ...
        if (isRestart) {
          ...
          if (respawn.mode === "spawned" || respawn.mode === "supervised") {
            ...
            if (respawn.mode === "failed") {
              ...
              defaultRuntime.log("gateway: restart mode in-process restart (KOLB_BOT_NO_RESPAWN)");
            }
            shuttingDown = false;
            restartResolver?.();
          }
...
void main().catch((err) => {
  console.error(
    "[kolb-bot] Gateway daemon failed:",
