...
export function registerDnsCli(program: Command) {
  const dns = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/dns", "github.com/kolbick/Kolb-Bot/blob/main/docs/cli/dns")}\n`,
    );
  ...
  dns
    ...
    .action(async (opts) => {
      ...
      defaultRuntime.log("");
      defaultRuntime.log(theme.heading("Recommended ~/.kolb-bot/kolb-bot.json:"));
      defaultRuntime.log(
        JSON.stringify(
          {
            gateway: { bind: "auto" },
      ...
      if (cfg.discovery?.wideArea?.enabled !== true) {
        ...
        defaultRuntime.log(
          theme.muted(
            "Note: enable discovery.wideArea.enabled in ~/.kolb-bot/kolb-bot.json on the gateway and restart the gateway so it writes the DNS-SD zone.",
          ),
        );
      }
    });
