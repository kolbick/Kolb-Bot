...
async function loadValidConfig() {
  ...
  defaultRuntime.error(`Run \`${formatCliCommand("kolb-bot doctor")}\` to repair, then retry.`);
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "github.com/kolbick/Kolb-Bot/blob/main/docs/cli/config")}\n`,
    )
    .option(
      "--section <section>",
