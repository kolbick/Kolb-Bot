...
describe("bundled extension resolver", () => {
  it("walks up to find the assets directory", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-ext-root-"));
...
describe("browser extension install", () => {
  it("installs into the state dir (never node_modules)", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-ext-"));
  ...
  it("copies extension path to clipboard", async () => {
    const prev = process.env.KOLB_BOT_STATE_DIR;
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "kolb-bot-ext-path-"));
    ...
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.KOLB_BOT_STATE_DIR = prev;
      }
    }
  });
});
