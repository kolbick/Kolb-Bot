...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    ...
    KOLB_BOT_CONFIG_PATH: process.env.KOLB_BOT_CONFIG_PATH,
    KOLB_BOT_GATEWAY_PORT: process.env.KOLB_BOT_GATEWAY_PORT,
    KOLB_BOT_PROFILE: process.env.KOLB_BOT_PROFILE,
  };
  ...
  afterEach(() => {
    if (originalEnv.KOLB_BOT_STATE_DIR !== undefined) {
      process.env.KOLB_BOT_STATE_DIR = originalEnv.KOLB_BOT_STATE_DIR;
    } else {
      delete process.env.KOLB_BOT_STATE_DIR;
    }

    if (originalEnv.KOLB_BOT_CONFIG_PATH !== undefined) {
      process.env.KOLB_BOT_CONFIG_PATH = originalEnv.KOLB_BOT_CONFIG_PATH;
    } else {
      delete process.env.KOLB_BOT_CONFIG_PATH;
    }

    if (originalEnv.KOLB_BOT_GATEWAY_PORT !== undefined) {
      process.env.KOLB_BOT_GATEWAY_PORT = originalEnv.KOLB_BOT_GATEWAY_PORT;
    } else {
      delete process.env.KOLB_BOT_GATEWAY_PORT;
    }

    if (originalEnv.KOLB_BOT_PROFILE !== undefined) {
      process.env.KOLB_BOT_PROFILE = originalEnv.KOLB_BOT_PROFILE;
    } else {
      delete process.env.KOLB_BOT_PROFILE;
    }
  });
