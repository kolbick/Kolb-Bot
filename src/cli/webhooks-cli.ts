...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "github.com/kolbick/Kolb-Bot/blob/main/docs/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + KolbBot hooks")
    ...
    .option("--hook-url <url>", "KolbBot hook URL")
    .option("--hook-token <token>", "KolbBot hook token")
