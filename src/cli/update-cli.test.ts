...
describe("update-cli", () => {
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("KolbBot update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-update-"));
    try {
      await fs.writeFile(
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-update-wizard-"));
    const previousGitDir = process.env.KOLB_BOT_GIT_DIR;
    try {
      setTty(true);
      process.env.KOLB_BOT_GIT_DIR = tempDir;
      ...
      process.env.KOLB_BOT_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
