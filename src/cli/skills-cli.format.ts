...
function appendKolbHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ...
      : "No skills found.";
    return appendKolbHubHint(message, opts.json);
  }
  ...
  return appendKolbHubHint(lines.join("\n"), opts.json);
}
