...
describe("restart-helper", () => {
  ...
  describe("prepareRestartScript", () => {
    ...
    it("creates a schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "KolbBot Gateway"');
      expect(content).toContain('schtasks /Run /TN "KolbBot Gateway"');
      // Batch self-cleanup
    ...
    it("uses KOLB_BOT_WINDOWS_TASK_NAME override on Windows", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        KOLB_BOT_PROFILE: "default",
        KOLB_BOT_WINDOWS_TASK_NAME: "KolbBot Gateway (custom)",
      });
      expect(content).toContain('schtasks /End /TN "KolbBot Gateway (custom)"');
      expect(content).toContain('schtasks /Run /TN "KolbBot Gateway (custom)"');
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain('schtasks /End /TN "KolbBot Gateway (production)"');
      await cleanupScript(scriptPath);
    });
