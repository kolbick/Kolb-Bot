...
async function ensureKolbBotExtensions(manifest: PackageManifest) {
  const extensions = manifest[MANIFEST_KEY]?.extensions;
  if (!Array.isArray(extensions)) {
    throw new Error("package.json missing kolb-bot.extensions");
  }
  const list = extensions.map((e) => (typeof e === "string" ? e.trim() : "")).filter(Boolean);
  if (list.length === 0) {
    throw new Error("package.json kolb-bot.extensions is empty");
  }
  return list;
}
...
async function installPluginFromPackageDir(params: {
  ...
  try {
    extensions = await ensureKolbBotExtensions(manifest);
  } catch (err) {
...
export async function installPluginFromArchive(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-plugin-"));
...
export async function installPluginFromNpmSpec(params: {
  ...
  const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-npm-pack-"));
