...
async function withStateDir<T>(stateDir: string, fn: () => Promise<T>) {
  ...
  const prevBundled = process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR;
  ...
  try {
    ...
    if (prevBundled === undefined) {
      ...
      process.env.KOLB_BOT_BUNDLED_PLUGINS_DIR = prevBundled;
    }
  }
}
...
describe("discoverKolbBotPlugins", () => {
  ...
  it("rejects package extension entries that escape via symlink", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@kolb-bot/pack",
        "kolb-bot": { extensions: ["./linked/escape.ts"] },
      }),
      "utf-8",
    );
