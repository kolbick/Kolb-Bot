...
- If you see "fatal: not a git repository", you are in the wrong directory. Use `~/dev/kolb-bot` if available; otherwise ask user.
...
Assume KolbBot subagents run with full disk access, including git, gh, and shell. Check auth, input validation, secrets, dependencies, tool safety, and privacy.
