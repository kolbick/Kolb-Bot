...
- **Task tool** in the upstream spec == KolbBot `sessions_spawn`
- **File I/O** == KolbBot `read`/`write`
- **Remote fetch** == KolbBot `web_fetch` (or `exec` with curl when POST is required)
