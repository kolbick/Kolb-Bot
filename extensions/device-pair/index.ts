...
import { approveDevicePairing, listDevicePairing } from "kolb-bot/plugin-sdk";
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the KolbBot iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the KolbBot iOS app:",
            "",
            "```",
            qrAscii,
            "```",
