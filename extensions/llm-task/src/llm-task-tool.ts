...
import Ajv from "ajv";
// NOTE: This extension is intended to be bundled with KolbBot.
// When running from source (tests/dev), KolbBot internals live under src/.
...
export function createLlmTaskTool(api: KolbBotPluginApi) {
  return {
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "kolb-bot-llm-task-"));
