...
KolbBot extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
kolb-bot message send --channel zalouser --target <threadId> --message "Hello from KolbBot!"
```
...
```bash
kolb-bot channels login --channel zalouser              # Login via QR
...
```bash
# Text
kolb-bot message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
kolb-bot message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `kolb-bot channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
kolb-bot channels login --channel zalouser --account work
kolb-bot message send --channel zalouser --account work --target <id> --message "Hello"
