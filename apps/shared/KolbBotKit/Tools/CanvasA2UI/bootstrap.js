...
import "@a2ui/lit/ui";
import { themeContext } from "@kolb-bot/a2ui-theme-context";
...
class KolbBotA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  static styles = css`
    :host {
      ...
      padding:
        var(--kolb-bot-a2ui-inset-top, 0px)
        var(--kolb-bot-a2ui-inset-right, 0px)
        var(--kolb-bot-a2ui-inset-bottom, 0px)
        var(--kolb-bot-a2ui-inset-left, 0px);
    }
    ...
    #surfaces {
      ...
      height: 100%;
      overflow: auto;
      padding-bottom: var(--kolb-bot-a2ui-scroll-pad-bottom, 0px);
    }
    ...
    .status {
      ...
      top: var(--kolb-bot-a2ui-status-top, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .toast {
      ...
      left: 50%;
      transform: translateX(-50%);
      bottom: var(--kolb-bot-a2ui-toast-bottom, 12px);
      display: inline-flex;
      align-items: center;
      gap: 8px;
      padding: 8px 10px;
    ...
    .empty {
      ...
      left: 50%;
      transform: translateX(-50%);
      top: var(--kolb-bot-a2ui-empty-top, var(--kolb-bot-a2ui-status-top, 12px));
      text-align: center;
      opacity: 0.8;
      padding: 10px 12px;
      pointer-events: none;
  ...
  #handleA2UIAction(evt) {
    ...
    globalThis.__kolbBotLastA2UIAction = userAction;

    const handler =
      globalThis.webkit?.messageHandlers?.kolbBotCanvasA2UIAction ??
      globalThis.kolbBotCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.kolbBotCanvasA2UIAction) {
...
if (!customElements.get("kolb-bot-a2ui-host")) {
  customElements.define("kolb-bot-a2ui-host", KolbBotA2UIHost);
}
