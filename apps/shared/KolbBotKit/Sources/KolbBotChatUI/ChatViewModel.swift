...
private let chatUILogger = Logger(subsystem: "ai.kolb-bot", category: "KolbBotChatUI")

@MainActor
@Observable
public final class KolbBotChatViewModel {
    public private(set) var messages: [KolbBotChatMessage] = []
    ...
    public var attachments: [KolbBotPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [KolbBotChatPendingToolCall] = []
    public private(set) var sessions: [KolbBotChatSessionEntry] = []
    private let transport: any KolbBotChatTransport
    ...
    private var pendingToolCallsById: [String: KolbBotChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any KolbBotChatTransport) {
    ...
    public var sessionChoices: [KolbBotChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [KolbBotChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [KolbBotChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: KolbBotPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [KolbBotChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: KolbBotChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [KolbBotChatMessage]) -> [KolbBotChatMessage] {
        var result: [KolbBotChatMessage] = []
    ...
    private static func dedupeKey(for message: KolbBotChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [KolbBotChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> KolbBotChatAttachmentPayload in
            KolbBotChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> KolbBotChatSessionEntry {
        KolbBotChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: KolbBotChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: KolbBotChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: KolbBotAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = KolbBotChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> KolbBotPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
