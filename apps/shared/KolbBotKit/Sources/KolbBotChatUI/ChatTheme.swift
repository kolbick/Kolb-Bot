...
enum KolbBotChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("KolbBotChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("KolbBotChatTheme.onboardingAssistantBubble"),
...
enum KolbBotPlatformImageFactory {
    static func image(_ image: KolbBotPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
