...
@MainActor
struct KolbBotChatComposer: View {
    @Bindable var viewModel: KolbBotChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(KolbBotChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(KolbBotChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \KolbBotPendingAttachment.id)
                { (att: KolbBotPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            KolbBotPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(KolbBotChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(KolbBotChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(KolbBotChatTheme.subtleCard)
        .clipShape(Capsule())
    }
