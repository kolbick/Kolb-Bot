...
struct ChatMessageBubble: View {
    let message: KolbBotChatMessage
    let style: KolbBotChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? KolbBotChatTheme.userText : KolbBotChatTheme.assistantText
    ...
    private var inlineAttachments: [KolbBotChatMessageContent] {
    ...
    private var toolCalls: [KolbBotChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [KolbBotChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? KolbBotChatTheme.userBubble
        }
        if self.style == .onboarding {
            return KolbBotChatTheme.onboardingAssistantBubble
        }
        return KolbBotChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return KolbBotChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: KolbBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? KolbBotChatTheme.userText : KolbBotChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: KolbBotChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(KolbBotChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(KolbBotChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [KolbBotChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: KolbBotChatTheme.assistantText)
            }
        }
    }
}
