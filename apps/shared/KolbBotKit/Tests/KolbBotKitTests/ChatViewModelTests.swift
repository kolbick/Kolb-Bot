...
private final class TestChatTransport: @unchecked Sendable, KolbBotChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [KolbBotChatHistoryPayload]
    private let sessionsResponses: [KolbBotChatSessionsListResponse]

    private let stream: AsyncStream<KolbBotChatTransportEvent>
    private let continuation: AsyncStream<KolbBotChatTransportEvent>.Continuation

    init(
        historyResponses: [KolbBotChatHistoryPayload],
        sessionsResponses: [KolbBotChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<KolbBotChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> KolbBotChatHistoryPayload {
        ...
        return self.historyResponses.last ?? KolbBotChatHistoryPayload(
        ...
        attachments _: [KolbBotChatAttachmentPayload]) async throws -> KolbBotChatSendResponse
    {
        ...
        return KolbBotChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> KolbBotChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? KolbBotChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: KolbBotChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = KolbBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = KolbBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { KolbBotChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = KolbBotChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = KolbBotChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = KolbBotChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { KolbBotChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
