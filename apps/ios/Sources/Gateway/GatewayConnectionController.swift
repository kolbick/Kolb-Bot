...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [KolbBotCapability.canvas.rawValue, KolbBotCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(KolbBotCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(KolbBotCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = KolbBotLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(KolbBotCapability.location.rawValue) }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            KolbBotSystemCommand.notify.rawValue,
            KolbBotSystemCommand.which.rawValue,
            KolbBotSystemCommand.run.rawValue,
            KolbBotSystemCommand.execApprovalsGet.rawValue,
            KolbBotSystemCommand.execApprovalsSet.rawValue,
        ]

        let caps = Set(self.currentCaps())
        if caps.contains(KolbBotCapability.camera.rawValue) {
            commands.append(KolbBotCameraCommand.list.rawValue)
            commands.append(KolbBotCameraCommand.snap.rawValue)
            commands.append(KolbBotCameraCommand.clip.rawValue)
        }
        if caps.contains(KolbBotCapability.location.rawValue) {
            commands.append(KolbBotLocationCommand.get.rawValue)
        }
