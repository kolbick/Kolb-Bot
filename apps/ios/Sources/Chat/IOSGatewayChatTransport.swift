...
struct IOSGatewayChatTransport: KolbBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KolbBotChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(KolbBotChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> KolbBotChatHistoryPayload {
        ...
        return try JSONDecoder().decode(KolbBotChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [KolbBotChatAttachmentPayload]) async throws -> KolbBotChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [KolbBotChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(KolbBotChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(KolbBotGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KolbBotGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KolbBotChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: KolbBotAgentEventPayload.self)
                        {
