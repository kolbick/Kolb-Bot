...
final class NodeAppModel {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = KolbBotCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = KolbBotCanvasA2UIAction.compactJSON(userAction["context"])
        ...
        let message = KolbBotCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = KolbBotCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    private func resolveA2UIHostURL() async -> String? {
        ...
        return base.appendingPathComponent("__kolb-bot__/a2ui/").absoluteString + "?platform=ios"
    }
    ...
    func requestLocationPermissions(mode: KolbBotLocationMode) async -> Bool {
    ...
    private func sendAgentRequest(link: AgentDeepLink) async throws {
        ...
        // (Key-based unattended auth is handled on macOS for kolb-bot:// links.)
    ...
    private func handleInvoke(_ req: BridgeInvokeRequest) async -> BridgeInvokeResponse {
        ...
        do {
            switch command {
            case KolbBotLocationCommand.get.rawValue:
                return try await self.handleLocationInvoke(req)
            case KolbBotCanvasCommand.present.rawValue,
                 KolbBotCanvasCommand.hide.rawValue,
                 KolbBotCanvasCommand.navigate.rawValue,
                 KolbBotCanvasCommand.evalJS.rawValue,
                 KolbBotCanvasCommand.snapshot.rawValue:
                return try await self.handleCanvasInvoke(req)
            case KolbBotCanvasA2UICommand.reset.rawValue,
                 KolbBotCanvasA2UICommand.push.rawValue,
                 KolbBotCanvasA2UICommand.pushJSONL.rawValue:
                return try await self.handleCanvasA2UIInvoke(req)
            case KolbBotCameraCommand.list.rawValue,
                 KolbBotCameraCommand.snap.rawValue,
                 KolbBotCameraCommand.clip.rawValue:
                return try await self.handleCameraInvoke(req)
            case KolbBotScreenCommand.record.rawValue:
                return try await self.handleScreenRecordInvoke(req)
            default:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: KolbBotNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: KolbBotNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(KolbBotLocationGetParams.self, from: req.paramsJSON)) ??
            KolbBotLocationGetParams()
        ...
        let payload = KolbBotLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case KolbBotCanvasCommand.present.rawValue:
            let params = (try? Self.decodeParams(KolbBotCanvasPresentParams.self, from: req.paramsJSON)) ??
                KolbBotCanvasPresentParams()
        ...
        case KolbBotCanvasCommand.hide.rawValue:
            return BridgeInvokeResponse(id: req.id, ok: true)
        case KolbBotCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(KolbBotCanvasNavigateParams.self, from: req.paramsJSON)
            self.screen.navigate(to: params.url)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case KolbBotCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(KolbBotCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KolbBotCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(KolbBotCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case KolbBotCanvasA2UICommand.reset.rawValue:
            ...
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case KolbBotCanvasA2UICommand.push.rawValue, KolbBotCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [AnyCodable]
            if command == KolbBotCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(KolbBotCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try KolbBotCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(KolbBotCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try KolbBotCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.kolb-botA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        case KolbBotCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KolbBotCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(KolbBotCameraSnapParams.self, from: req.paramsJSON)) ??
                KolbBotCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case KolbBotCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(KolbBotCameraClipParams.self, from: req.paramsJSON)) ??
                KolbBotCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(KolbBotScreenRecordParams.self, from: req.paramsJSON)) ??
            KolbBotScreenRecordParams()
...
private extension NodeAppModel {
    func locationMode() -> KolbBotLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return KolbBotLocationMode(rawValue: raw) ?? .off
    }
