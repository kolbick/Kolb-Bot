package ai.kolb-bot.android.node
...
import ai.kolb-bot.android.ScreenCaptureRequester
...
class ScreenRecordManager(private val context: Context) {
  ...
  @Volatile private var permissionRequester: ai.kolb-bot.android.PermissionRequester? = null

  fun attachScreenCaptureRequester(requester: ScreenCaptureRequester) {
    screenCaptureRequester = requester
  }

  fun attachPermissionRequester(requester: ai.kolb-bot.android.PermissionRequester) {
  ...
  suspend fun record(paramsJson: String?): Payload =
    withContext(Dispatchers.Default) {
      ...
      val file = File.createTempFile("kolb-bot-screen-", ".mp4")
