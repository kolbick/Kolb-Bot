...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/KolbBot/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    let preferred = home
        .appendingPathComponent("Library/Application Support/KolbBot/control.sock")
        .path
    return preferred
}()
