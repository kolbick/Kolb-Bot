...
public final class GatewayDiscoveryModel {
    ...
    private let logger = Logger(subsystem: "ai.kolb-bot", category: "gateway-discovery")
    ...
    public func start() {
        ...
        for domain in KolbBotBonjour.gatewayServiceDomains {
            let params = NWParameters.tcp
            params.includePeerToPeer = true
            let browser = NWBrowser(
                for: .bonjour(type: KolbBotBonjour.gatewayServiceType, domain: domain),
                using: params)
            ...
            browser.start(queue: DispatchQueue(label: "ai.kolb-bot.macos.gateway-discovery.\(domain)"))
        }
    ...
    public func refreshWideAreaFallbackNow(timeoutSeconds: TimeInterval = 5.0) {
        guard let domain = KolbBotBonjour.wideAreaGatewayServiceDomain else { return }
    ...
    private func updateGateways(for domain: String) {
        ...
        if let wideAreaDomain = KolbBotBonjour.wideAreaGatewayServiceDomain,
    ...
    private var hasUsableWideAreaResults: Bool {
        guard let domain = KolbBotBonjour.wideAreaGatewayServiceDomain else { return false }
    ...
    private nonisolated static func prettifyInstanceName(_ decodedName: String) -> String {
        ...
        let stripped = normalized.replacingOccurrences(of: " (KolbBot)", with: "")
