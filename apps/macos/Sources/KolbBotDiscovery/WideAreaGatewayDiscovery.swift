...
enum WideAreaGatewayDiscovery {
        ...
        context: DiscoveryContext = .live) -> [WideAreaGatewayBeacon]
    {
        ...
        let probeName = "_kolb-bot-gw._tcp.\(domainTrimmed)"
        ...
        for raw in ptrLines {
            ...
            let suffix = "._kolb-bot-gw._tcp.\(domainTrimmed)"
    ...
    {
        guard let domain = KolbBotBonjour.wideAreaGatewayServiceDomain else { return nil }
        let domainTrimmed = domain.trimmingCharacters(in: CharacterSet(charactersIn: "."))
        let probeName = "_kolb-bot-gw._tcp.\(domainTrimmed)"
