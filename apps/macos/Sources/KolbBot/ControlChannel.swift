...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: KolbBotProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.kolb-bot", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: KolbBotKit.AnyCodable]()) {
                $0[$1.key] = KolbBotKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: KolbBotProtocol.AnyCodable?) -> [String: KolbBotProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: KolbBotProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: KolbBotKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: KolbBotProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("kolb-bot.control.heartbeat")
    static let controlAgentEvent = Notification.Name("kolb-bot.control.agent")
}
