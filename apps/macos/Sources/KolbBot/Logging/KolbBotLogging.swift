...
enum KolbBotLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = KolbBotOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = KolbBotFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        KolbBotLogging.bootstrapIfNeeded()
        let label = KolbBotLogging.makeLabel(subsystem: subsystem, category: category)
...
struct KolbBotOSLogHandler: LogHandler {
...
struct KolbBotFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = KolbBotLogging.parseLabel(self.label)
