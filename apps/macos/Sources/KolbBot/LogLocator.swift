...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["KOLB_BOT_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/kolb-bot")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("kolb-bot-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("kolb-bot-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/kolb-bot/ (rolling or stdout), or nil if none exist.
