    ...
    private var launchdSection: some View {
        GroupBox("Gateway startup") {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(
                    "When enabled, KolbBot won't install or manage \(gatewayLaunchdLabel). " +
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                HStack(spacing: 8) {
                    ...
                    Button("Copy sample URL") {
                        ...
                        let url = "kolb-bot://agent?message=\(encoded)&key=\(key)"
                        NSPasteboard.general.clearContents()
                ...
                Text("Deep links (kolb-bot://…) are always enabled; the key controls unattended runs.")
                    .font(.caption2)
                    .foregroundStyle(.secondary)
                ...
                GridRow {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Toggle("Write rolling diagnostics log (JSONL)", isOn: self.$diagnosticsFileLogEnabled)
                            ...
                            .help(
                                "Writes a rotating, local-only log under ~/Library/Logs/KolbBot/. " +
    ...
    private var pathsSection: some View {
        GroupBox("Paths") {
            VStack(alignment: .leading, spacing: 12) {
                VStack(alignment: .leading, spacing: 6) {
                    Text("KolbBot project root")
                        .font(.caption.weight(.semibold))
                    HStack(spacing: 8) {
                        TextField("Path to kolb-bot repo", text: self.$gatewayRootInput)
                        ...
                        Button("Reset") {
                            let def = FileManager().homeDirectoryForCurrentUser
                                .appendingPathComponent("Projects/kolb-bot").path
                    ...
                    GridRow {
                        ...
                        VStack(alignment: .leading, spacing: 6) {
                            ...
                            if let sessionStoreSaveError {
                                ...
                                Text("Used by the CLI session loader; stored in ~/.kolb-bot/kolb-bot.json.")
                                    .font(.footnote)
                                    .foregroundStyle(.secondary)
                            }
                        }
    ...
    private var quickActionsSection: some View {
        GroupBox("Quick actions") {
            VStack(alignment: .leading, spacing: 10) {
                ...
                VStack(alignment: .leading, spacing: 6) {
                    Text(
                        "Note: macOS may require restarting KolbBot after enabling Accessibility or Screen Recording.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                        .fixedSize(horizontal: false, vertical: true)

                    Button {
                        LaunchdManager.startKolbBot()
                    } label: {
                        Label("Restart KolbBot", systemImage: "arrow.counterclockwise")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                }
...
extension DebugSettings {
    static func exerciseForTesting() async {
        ...
        view.gatewayRootInput = "/tmp/kolb-bot"
