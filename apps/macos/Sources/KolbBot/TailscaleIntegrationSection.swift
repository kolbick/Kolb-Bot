...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.kolb-bot/kolb-bot.json. Prefer KOLB_BOT_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.kolb-bot/kolb-bot.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.kolb-bot/kolb-bot.json. Restart the gateway to apply."
        }
