...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so KolbBot can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = KolbBotLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = KolbBotLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(KolbBotLocationMode.off.rawValue)
                Text("While Using").tag(KolbBotLocationMode.whileUsing.rawValue)
                Text("Always").tag(KolbBotLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = KolbBotLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: KolbBotLocationMode {
        KolbBotLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: KolbBotLocationMode) async -> Bool {
