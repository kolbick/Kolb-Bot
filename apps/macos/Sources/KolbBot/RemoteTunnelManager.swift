...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.kolb-bot", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous KolbBot run already has an SSH listener on the expected port (common after restarts),
