...
private let gatewayConnectionLogger = Logger(subsystem: "ai.kolb-bot", category: "gateway.connection")
...
actor GatewayConnection {
    ...
    private func sessionDefaultString(_ defaults: [String: KolbBotProtocol.AnyCodable]?, key: String) -> String {
...
extension GatewayConnection {
    ...
    func healthOK(timeoutMs: Int = 8000) async throws -> Bool {
        ...
        return (try? self.decoder.decode(KolbBotGatewayHealthOK.self, from: data))?.ok ?? true
    }
    ...
    func sessionsPreview(
        ...
        limit: Int? = nil,
        maxChars: Int? = nil,
        timeoutMs: Int? = nil) async throws -> KolbBotSessionsPreviewPayload
    {
        let resolvedKeys = keys
            .map { self.canonicalizeSessionKey($0) }
            .filter { !$0.isEmpty }
        if resolvedKeys.isEmpty {
            return KolbBotSessionsPreviewPayload(ts: 0, previews: [])
        }
    ...
    func chatHistory(
        sessionKey: String,
        limit: Int? = nil,
        timeoutMs: Int? = nil) async throws -> KolbBotChatHistoryPayload
    {
    ...
    func chatSend(
        ...
        attachments: [KolbBotChatAttachmentPayload],
        timeoutMs: Int = 30000) async throws -> KolbBotChatSendResponse
    {
        let resolvedKey = self.canonicalizeSessionKey(sessionKey)
        var params: [String: AnyCodable] = [
