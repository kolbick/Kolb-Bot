...
private let deepLinkLogger = Logger(subsystem: "ai.kolb-bot", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "KolbBot is paused", message: "Unpause KolbBot to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run KolbBot agent?", message: body) else { return }
        }
