...
private let webChatSwiftLogger = Logger(subsystem: "ai.kolb-bot", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: KolbBotChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> KolbBotChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(KolbBotChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> KolbBotChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                KolbBotGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<KolbBotChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any KolbBotChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = KolbBotChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: KolbBotChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<KolbBotChatView>) -> NSViewController
    {
