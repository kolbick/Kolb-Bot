...
enum DebugActions {
    private static let verboseDefaultsKey = "kolb-bot.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "kolb-bot.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = KolbBotPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "KolbBot", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = KolbBotPaths.configURL
        guard
