...
enum CommandResolver {
    private static let projectRootDefaultsKey = "kolb-bot.gatewayProjectRootPath"
    private static let helperName = "kolb-bot"
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/kolb-bot")
    ...
    static func projectKolbBotExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("kolb-bot.mjs").path,
            root.appendingPathComponent("bin/kolb-bot.js").path,
        ]
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            let missingEntry = """
            kolb-bot entrypoint missing (looked for dist/index.js or kolb-bot.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real kolb-bot CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/kolb-bot"
        ...
        \(cliSection)
        \(projectSection)
        if command -v kolb-bot >/dev/null 2>&1; then
          CLI="$(command -v kolb-bot)"
          kolb-bot \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/kolb-bot.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/kolb-bot.mjs"
            node "$PRJ/kolb-bot.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/kolb-bot.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/kolb-bot.js"
            node "$PRJ/bin/kolb-bot.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent kolb-bot"
          pnpm --silent kolb-bot \(quotedArgs);
        else
          echo "kolb-bot CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? KolbBotConfigFile.loadDict()
