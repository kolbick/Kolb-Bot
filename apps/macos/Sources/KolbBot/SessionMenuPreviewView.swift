...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.kolb-bot", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> KolbBotSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: KolbBotSessionPreviewEntry,
    ...
    private static func cache(payload: KolbBotSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: KolbBotChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [KolbBotKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [KolbBotKit.AnyCodable]) -> [KolbBotChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(KolbBotChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: KolbBotChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: KolbBotChatMessage) -> Bool {
    ...
    private static func toolNames(for message: KolbBotChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: KolbBotChatMessage) -> String? {
