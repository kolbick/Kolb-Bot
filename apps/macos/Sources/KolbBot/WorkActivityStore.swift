...
final class WorkActivityStore {
    ...
    func handleTool(
        ...
        name: String?,
        meta: String?,
        args: [String: KolbBotProtocol.AnyCodable]?)
    {
    ...
    private static func buildLabel(
        name: String?,
        meta: String?,
        args: [String: KolbBotProtocol.AnyCodable]?) -> String
    {
    ...
    private static func wrapToolArgs(_ args: [String: KolbBotProtocol.AnyCodable]?) -> KolbBotKit.AnyCodable? {
        guard let args else { return nil }
        let converted: [String: Any] = args.mapValues { self.unwrapJSONValue($0.value) }
        return KolbBotKit.AnyCodable(converted)
    }

    private static func unwrapJSONValue(_ value: Any) -> Any {
        if let dict = value as? [String: KolbBotProtocol.AnyCodable] {
            return dict.mapValues { self.unwrapJSONValue($0.value) }
        }
        if let array = value as? [KolbBotProtocol.AnyCodable] {
