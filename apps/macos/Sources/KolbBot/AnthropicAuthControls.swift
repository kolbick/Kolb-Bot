...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: KolbBotOAuthStore.AnthropicOAuthStatus = KolbBotOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: KolbBotOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = KolbBotOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = KolbBotOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try KolbBotOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. KolbBot can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: KolbBotOAuthStore.AnthropicOAuthStatus,
