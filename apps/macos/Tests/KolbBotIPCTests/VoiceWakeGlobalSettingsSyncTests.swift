...
@Suite(.serialized) struct VoiceWakeGlobalSettingsSyncTests {
    @Test func appliesVoiceWakeChangedEventToAppState() async {
        ...
        let payload = KolbBotProtocol.AnyCodable(["triggers": ["kolb-bot", "computer"]])
        let evt = EventFrame(
            type: "event",
        ...
        #expect(updated == ["kolb-bot", "computer"])
    ...
    @Test func ignoresVoiceWakeChangedEventWithInvalidPayload() async {
        ...
        let payload = KolbBotProtocol.AnyCodable(["unexpected": 123])
        let evt = EventFrame(
            type: "event",
            event: "voicewake.changed",
            payload: payload,
