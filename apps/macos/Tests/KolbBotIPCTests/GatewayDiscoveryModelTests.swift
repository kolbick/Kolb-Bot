...
struct GatewayDiscoveryModelTests {
    ...
    @Test func localGatewayMatchesDisplayName() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (KolbBot)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func serviceNameDoesNotFalsePositiveOnSubstringHostToken() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (KolbBot)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (KolbBot)",
            local: local))
    }
    ...
    @Test func parsesGatewayTXTFields() {
        ...
        #expect(parsed.cliPath == "/opt/kolb-bot")
    }
