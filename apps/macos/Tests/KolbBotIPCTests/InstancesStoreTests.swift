...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: KolbBotProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: KolbBotProtocol.AnyCodable] = [
            "presence": .init([KolbBotProtocol.AnyCodable(entry)]),
        ]
        let payload = KolbBotProtocol.AnyCodable(payloadMap)
