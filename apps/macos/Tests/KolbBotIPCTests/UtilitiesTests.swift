...
@Suite(.serialized) struct UtilitiesTests {
    ...
    @Test func gatewayEntrypointPrefersDistOverBin() throws {
        ...
        let bin = tmp.appendingPathComponent("bin/kolb-bot.js")
    ...
    @Test func logLocatorPicksNewestLogFile() throws {
        let fm = FileManager()
        let dir = URL(fileURLWithPath: "/tmp/kolb-bot", isDirectory: true)
        try? fm.createDirectory(at: dir, withIntermediateDirectories: true)

        let older = dir.appendingPathComponent("kolb-bot-old-\(UUID().uuidString).log")
        let newer = dir.appendingPathComponent("kolb-bot-new-\(UUID().uuidString).log")
