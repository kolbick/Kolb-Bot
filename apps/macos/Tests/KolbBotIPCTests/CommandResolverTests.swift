...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersKolbBotBinary() async throws {
        ...
        let kolb-botPath = tmp.appendingPathComponent("node_modules/.bin/kolb-bot")
        try self.makeExec(at: kolb-botPath)

        let cmd = CommandResolver.kolb-botCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([kolb-botPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/kolb-bot.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.kolb-botCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.kolb-botCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])

        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "kolb-bot", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "kolb-bot", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("kolb-bot@example.com:2222", forKey: remoteTargetKey)
        defaults.set("/tmp/id_ed25519", forKey: remoteIdentityKey)
        defaults.set("/srv/kolb-bot", forKey: remoteProjectRootKey)
        ...
        if let marker = cmd.firstIndex(of: "--") {
            #expect(cmd[marker + 1] == "kolb-bot@example.com")
        } else {
            #expect(Bool(false))
        }
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/kolb-bot'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("kolb-bot"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == kolb-botPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
