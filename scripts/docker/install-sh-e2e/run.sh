...
MODELS_MODE="${KOLB_BOT_E2E_MODELS:-both}" # both|openai|anthropic
INSTALL_TAG="${KOLB_BOT_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${KOLB_BOT_INSTALL_E2E_PREVIOUS:-}"
SKIP_PREVIOUS="${KOLB_BOT_INSTALL_E2E_SKIP_PREVIOUS:-0}"
...
if [[ "$INSTALL_TAG" == "beta" ]]; then
  KOLB_BOT_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  KOLB_BOT_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
