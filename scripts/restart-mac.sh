#!/usr/bin/env bash
# Reset KolbBot like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${KOLB_BOT_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="KolbBot.app/Contents/MacOS/KolbBot"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/KolbBot"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/KolbBot"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/KolbBot"
...
log "==> Killing existing KolbBot instances"
kill_all_kolb-bot
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product KolbBot"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/KolbBot.app" ]]; then
    APP_BUNDLE="/Applications/KolbBot.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/KolbBot.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/KolbBot.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/KolbBot.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set KOLB_BOT_APP_BUNDLE to your installed KolbBot.app"
}
