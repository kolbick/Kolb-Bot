...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "KolbBot")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "KolbBot")
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/KolbBot/Resources/KolbBot.icns"
  if [[ -f "$ICON_SRC" ]]; then
