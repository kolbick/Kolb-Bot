...
ZIP=${1:?"Usage: $0 KolbBot-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/kolb-bot/kolb-bot/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^KolbBot-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/kolb-bot/kolb-bot/releases/download/v${VERSION}/"}
