...
# Build and bundle KolbBot into a minimal .app we can open.
# Outputs to dist/KolbBot.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/KolbBot.app"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/kolb-bot/kolb-bot/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/KolbBot/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :KolbBotBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :KolbBotGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/KolbBot"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/KolbBot"
fi
chmod +x "$APP_ROOT/Contents/MacOS/KolbBot"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/KolbBot" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/KolbBot/Resources/KolbBot.icns" "$APP_ROOT/Contents/Resources/KolbBot.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/KolbBot/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
KOLB_BOTKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/KolbBotKit_KolbBotKit.bundle"
if [ -d "$KOLB_BOTKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/KolbBotKit_KolbBotKit.bundle"
  cp -R "$KOLB_BOTKIT_BUNDLE" "$APP_ROOT/Contents/Resources/KolbBotKit_KolbBotKit.bundle"
else
  echo "WARN: KolbBotKit resource bundle not found at $KOLB_BOTKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running KolbBot"
killall -q KolbBot 2>/dev/null || true
