...
import type { ApiContributor, Entry, MapConfig, User } from "./update-kolbtributors.types.js";

const REPO = "kolb-bot/kolb-bot";
const PER_LINE = 10;

const mapPath = resolve("scripts/kolbtributors-map.json");
...
if (start === -1 || end === -1) {
  throw new Error("README.md missing kolbtributors block");
}

const next = `${readme.slice(0, start)}<p align="left">\n${block}${readme.slice(end)}`;
writeFileSync(readmePath, next);

console.log(`Updated README kolbtributors: ${entries.length} entries`);
