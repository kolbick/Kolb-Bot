...
#   NOTIFY_PHONE - Phone number to send KolbBot notification (e.g., +1234567890)
...
send_notification() {
    ...
    # Send via KolbBot if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        ...
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via KolbBot to $NOTIFY_PHONE..."
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! KolbBot is down. Run: ssh l36 '~/kolb-bot/scripts/mobile-reauth.sh'" "urgent"
    exit 1
