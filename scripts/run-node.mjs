...
const logRunner = (message) => {
  if (env.KOLB_BOT_RUNNER_LOG === "0") {
    return;
  }
  process.stderr.write(`[kolb-bot] ${message}\n`);
};

const runNode = () => {
  const nodeProcess = spawn(process.execPath, ["kolb-bot.mjs", ...args], {
    cwd,
    env,
    stdio: "inherit",
  });
