#!/usr/bin/env bash
# ClawDock - Docker helpers for KolbBot
# Inspired by Simon Willison's "Running KolbBot in Docker"
# https://til.simonwillison.net/llms/kolb-bot-docker
#
# Installation:
#   mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/kolb-bot/kolb-bot/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    ...
    if [[ "$response" =~ ^[Nn] ]]; then
      ...
      echo "  export CLAWDOCK_DIR=/path/to/kolb-bot"
      return 1
    fi
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^KOLB_BOT_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d kolb-bot-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart kolb-bot-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f kolb-bot-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec kolb-bot-gateway \
...
clawdock-cli() {
  _clawdock_compose run --rm kolb-bot-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build kolb-bot-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "KOLB_BOT_GATEWAY_TOKEN=$token" kolb-bot-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" kolb-bot-gateway \
    bash -c './kolb-bot.mjs config set gateway.remote.token "$TOKEN" && ./kolb-bot.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec kolb-bot-gateway \
    bash -c "./kolb-bot.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart kolb-bot-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm kolb-bot-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec kolb-bot-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for KolbBot${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (kolb-bot alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to kolb-bot project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.kolb-bot)${_CLR_RESET}"
