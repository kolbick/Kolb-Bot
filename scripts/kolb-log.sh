...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "kolb-log needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
kolb-log - KolbBot Logging Utility

USAGE:
    kolb-log [OPTIONS]

DESCRIPTION:
    View KolbBot logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    KolbBot logs flow through the macOS unified log (subsystem: ai.kolb-bot).
...
QUICK START:
    kolb-log -n 100             Show last 100 lines from all components
    kolb-log -f                 Follow logs in real-time
    kolb-log -e                 Show only errors
    kolb-log -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    kolb-log                   Show last 50 lines from past 5 minutes (default)
    kolb-log -f                Stream logs continuously
    kolb-log -n 100            Show last 100 lines
    kolb-log -e                Show only recent errors
    kolb-log -l 30m -n 200     Show last 200 lines from past 30 minutes
    kolb-log -c ServerManager  Show recent ServerManager logs
    kolb-log -s "fail"         Search for "fail" in recent logs
    kolb-log --server -e       Show recent server errors
    kolb-log -f -d             Stream debug logs continuously
