...
# Checks both Claude Code and KolbBot auth status
...
echo ""
echo "KolbBot Auth (~/.kolb-bot/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
...
if systemctl --user is-active kolb-bot >/dev/null 2>&1; then
    echo -e "KolbBot service: ${GREEN}running${NC}"
else
    echo -e "KolbBot service: ${RED}NOT running${NC}"
fi
